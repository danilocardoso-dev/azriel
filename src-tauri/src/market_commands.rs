...
fn worker_connection(path: &std::path::Path) -> Result<rusqlite::Connection, String> {
    let mut connection = rusqlite::Connection::open(path).map_err(|error| error.to_string())?;
...
fn config_probe_input(agent_id: String) -> MarketExperimentInput {
    MarketExperimentInput {
        name: "probe".into(),
        dataset_id: String::new(),
        risk_profile_id: String::new(),
...
#[tauri::command]
pub async fn get_market_ai_status(
    state: State<'_, DatabaseState>,
    agent_id: Option<String>,
) -> Result<MarketAiStatus, String> {
    let requested = agent_id.unwrap_or_else(|| crate::database::market_ai::AI_AGENT_V4_ID.into());
    let (config, endpoint) = {
        let connection = lock(&state)?;
        market_repository::resolve_ai_config(&connection, &config_probe_input(requested))?
    };
    let health = ollama::status(&endpoint, (config.timeout_ms / 1000).clamp(5, 8)).await?;
    let model_available =
        health.available && health.models.iter().any(|model| model == &config.model);
    let error = if health.available && !model_available {
        Some(format!(
...
#[tauri::command]
pub fn update_market_ai_config(
    state: State<'_, DatabaseState>,
    input: UpdateMarketAiConfigInput,
) -> Result<MarketAiStatus, String> {
    let connection = lock(&state)?;
    market_repository::update_ai_config(&connection, &input)?;
    let (config, _) =
        market_repository::resolve_ai_config(&connection, &config_probe_input(input.agent_id))?;
    Ok(MarketAiStatus {
        agent_id: config.agent_id,
        configured: true,
        available: false,
...
#[tauri::command]
pub fn list_market_ai_runtime(
    state: State<'_, DatabaseState>,
    experiment_id: String,
) -> Result<Vec<MarketAiRuntimeMetric>, String> {
    let connection = lock(&state)?;
    market_repository::list_ai_runtime(&connection, &experiment_id)
}

#[tauri::command]
pub fn list_market_validation_ai_runtime(
    state: State<'_, DatabaseState>,
    validation_id: String,
) -> Result<Vec<MarketAiRuntimeMetric>, String> {
    let connection = lock(&state)?;
    market_repository::list_validation_ai_runtime(&connection, &validation_id)
}

#[tauri::command]
pub fn list_market_ai_decisions(
    state: State<'_, DatabaseState>,
    experiment_id: String,
) -> Result<Vec<MarketAiDecisionLog>, String> {
    let connection = lock(&state)?;
    market_repository::list_ai_decisions(&connection, &experiment_id)
}

#[tauri::command]
pub fn list_market_ai_experiment_comparisons(
    state: State<'_, DatabaseState>,
) -> Result<Vec<MarketAiExperimentComparison>, String> {
    let connection = lock(&state)?;
    market_repository::list_ai_experiment_comparisons(&connection)
}

#[tauri::command]
pub fn get_market_signal_diagnostics(
    state: State<'_, DatabaseState>,
    experiment_id: String,
