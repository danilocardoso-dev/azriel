...
pub fn persist_aggregates(
    ...
    for row in rows {
        ...
        metric.style = style;
        if intent == "ENTER" {
            if metric.style == "TREND" {
                metric.trend_entries += 1;
            }
            if metric.style == "MOMENTUM" {
                metric.breakout_entries += 1;
            }
            if metric.style == "MEAN_REVERSION" {
                metric.mean_reversion_entries += 1;
            }
        }
        if reason == "EMA_CROSS_BULLISH" {
            metric.ema_cross_entries += 1;
        }
        if reason.starts_with("BREAKOUT_") {
            metric.breakout_attempts += 1;
        }
        if reason == "BREAKOUT_WEAK_VOLUME" || reason == "MOMENTUM_FADE" {
            metric.failed_breakouts += 1;
        }
        if reason == "VWAP_DEVIATION" {
            metric.vwap_deviation_events += 1;
        }
        if reason == "MEAN_REVERSION_CONFIRMED" && intent == "EXIT" {
            metric.successful_reversions += 1;
        }
    }
    drop(statement);
    ...
    let holding_rows = holding_statement
        .query_map([experiment_id], |row| {
    ...
    let phase_rows = phase_statement
        .query_map([experiment_id], |row| {
            Ok((
                row.get::<_, String>(0)?,
                row.get::<_, String>(1)?,
    ...
    for row in phase_rows {
        let (agent, phase, pnl) = row.map_err(|error| error.to_string())?;
        phases.entry((agent, phase)).or_default().push(pnl);
    }
    drop(phase_statement);
    for ((agent, phase), pnl) in phases {
        let trades = pnl.len();
        let wins = pnl.iter().filter(|value| **value > 0.0).count();
        let sum = pnl.iter().sum::<f64>();
    ...
    for left in 0..agents.len() {
        for right in left + 1..agents.len() {
            ...
            let pairs = pair_statement
                .query_map(params![experiment_id, agents[left], agents[right]], |row| {
                    Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?))
                })
                .map_err(|error| error.to_string())?
                .collect::<Result<Vec<_>, _>>()
                .map_err(|error| error.to_string())?;
            let comparable = pairs.len();
            let same = pairs.iter().filter(|(a, b)| a == b).count();
            let entries = pairs
                .iter()
                .filter(|(a, b)| a == "BUY" && b == "BUY")
                .count();
            let exits = pairs
                .iter()
                .filter(|(a, b)| a == "SELL" && b == "SELL")
                .count();
...
pub fn load(
    ...
    experiment_id: &str,
) -> Result<MarketIntradayStrategyReport, String> {
    ...
    let feature_traces = statement
        .query_map([experiment_id], |row| {
            Ok(MarketIntradayFeatureTrace {
                candle_index: row.get(0)?,
                timestamp_utc: row.get(1)?,
                session_id: row.get(2)?,
                session_phase: row.get(3)?,
                session_progress: row.get(4)?,
                ready: row.get(5)?,
    ...
    let decisions = statement
        .query_map([experiment_id], |row| {
            Ok(MarketIntradayStrategyDecision {
                ...
                reason: row.get(7)?,
                indicators: parse_json(row.get(8)?)?,
                thresholds: parse_json(row.get(9)?)?,
                position_before: row.get(10)?,
                generated_target_exposure_pct: row.get(11)?,
                risk_result: row.get(12)?,
                execution_price: row.get(13)?,
            })
        })
        .map_err(|error| error.to_string())?
        .collect::<Result<Vec<_>, _>>()
    ...
    Ok(MarketIntradayStrategyReport {
