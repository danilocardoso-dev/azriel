...
pub fn evaluate(
...
) -> Result<MarketSignalSummary, String> {
    config.validate()?;
    let trend = average(
        [
            normalized(input.price_vs_sma_short_pct, config.trend_normalization_pct),
            normalized(
                input.price_vs_sma_long_pct,
                config.trend_normalization_pct * 1.5,
            ),
    ...
    let volatility = input
        .rolling_volatility
        .filter(|v| v.is_finite())
        .map(|v| (v / config.high_volatility).clamp(0.0, 1.0))
        .unwrap_or(0.0);
    let directional_weight = config.trend_weight + config.momentum_weight;
    let bullish = (config.trend_weight * trend.max(0.0)
        + config.momentum_weight * momentum.max(0.0))
        / directional_weight;
    let bearish = (config.trend_weight * (-trend).max(0.0)
        + config.momentum_weight * (-momentum).max(0.0))
        / directional_weight;
    let conflict = if trend.signum() != momentum.signum() {
        trend.abs().min(momentum.abs())
    } else {
        0.0
    };
    let conflict_level = if conflict >= config.conflict_threshold {
        ConflictLevel::High
    } else if conflict >= config.weak_signal_threshold {
        ConflictLevel::Medium
    } else {
        ConflictLevel::Low
    };
    let strength =
        ((bullish - bearish).abs() * (1.0 - config.volatility_weight * volatility)).clamp(0.0, 1.0);
    let bias = if strength < config.weak_signal_threshold {
    ...
    Ok(MarketSignalSummary {
        engine_version: SIGNAL_ENGINE_VERSION.into(),
...
mod tests {
    use super::*;
    fn aligned(sign: f64) -> MarketSignalInput {
        MarketSignalInput {
            price_vs_sma_short_pct: Some(3.0 * sign),
            price_vs_sma_long_pct: Some(5.0 * sign),
            sma_spread_pct: Some(3.0 * sign),
            sma_short_slope_pct: Some(2.0 * sign),
            sma_long_slope_pct: Some(1.5 * sign),
            return_1_pct: Some(2.0 * sign),
            return_5_pct: Some(5.0 * sign),
            return_10_pct: Some(8.0 * sign),
            return_20_pct: Some(12.0 * sign),
            rolling_volatility: Some(0.01),
            ..Default::default()
        }
    }
    #[test]
    fn aligned_trend_and_momentum_are_directional_and_normalized() {
        for (sign, expected) in [
            (1.0, DirectionalBias::Bullish),
            (-1.0, DirectionalBias::Bearish),
        ] {
            let s = evaluate(&aligned(sign), &Default::default()).unwrap();
            assert_eq!(s.directional_bias, expected);
            assert!(s.signal_strength >= 0.65);
            assert!((-1.0..=1.0).contains(&s.trend_score));
            assert!((-1.0..=1.0).contains(&s.momentum_score));
        }
    }
    #[test]
    fn opposing_components_create_high_conflict() {
        let mut input = aligned(1.0);
        input.return_1_pct = Some(-2.0);
        input.return_5_pct = Some(-5.0);
        input.return_10_pct = Some(-8.0);
        input.return_20_pct = Some(-12.0);
        let s = evaluate(&input, &Default::default()).unwrap();
        assert_eq!(s.conflict_level, ConflictLevel::High);
        assert_eq!(s.directional_bias, DirectionalBias::Neutral);
    }
    #[test]
    fn volatility_is_context_not_direction() {
        let mut low = aligned(1.0);
        low.rolling_volatility = Some(0.0);
        let mut high = low.clone();
        high.rolling_volatility = Some(0.03);
        let a = evaluate(&low, &Default::default()).unwrap();
        let b = evaluate(&high, &Default::default()).unwrap();
        assert_eq!(a.directional_bias, b.directional_bias);
        assert!(b.signal_strength < a.signal_strength);
    }
    #[test]
    fn configuration_is_versioned_and_validated() {
        let mut config = SignalEngineConfig::default();
        assert_eq!(config.version, SIGNAL_CONFIG_VERSION);
        config.trend_weight = 0.8;
