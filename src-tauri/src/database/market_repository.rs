use super::{ai_repository, market_ai, market_models::*, market_observatory, market_regimes};
...
#[derive(Debug, Clone)]
pub(crate) struct DecisionRow { pub(crate) candle_index: usize, timestamp: String, observed_price: f64, pub(crate) decision: Decision, risk_result: String, risk_reason: String, approved_pct: Option<f64>, cash_before: f64, equity_before: f64, exposure_before: f64, pub(crate) execution: Option<ExecutionRow>, ai: Option<market_ai::MarketAiDecision> }
        ...
        for run in &mut runs {
            ...
            let ai = if market_ai::is_ai_agent(&run.id) {
                let outcome = if (index - start) % ai_config.decision_interval == 0 {
                    if ai_config.prompt_version == market_ai::PROMPT_V2_VERSION {
            ...
            let (result, reason, approved) = if market_ai::is_ai_agent(&run.id) && decision.action == "SELL" && run.portfolio.quantity <= 0.0 && !profile.allow_short {
            ...
            run.decisions.push(DecisionRow { candle_index: index, timestamp: candle.timestamp.clone(), observed_price: candle.close, decision: decision.clone(), risk_result: result.clone(), risk_reason: reason, approved_pct: approved, cash_before: run.portfolio.cash, equity_before: before, exposure_before: exp, execution: None, ai });
