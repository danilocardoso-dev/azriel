...
pub fn output_schema_intent(reason_codes: &[&str]) -> Value {
    json!({
        "type": "object",
        "additionalProperties": false,
        "properties": {
...
fn extract_single_object(input: &str) -> Result<(String, bool), ValidationFailure> {
        ...
        if character == '"' {
            ...
            if depth == 0 {
                return Err(failure(
                    ...
                    None,
                    "Chave de fechamento sem objeto correspondente",
                    false,
                ));
            }
            depth -= 1;
    ...
    if spans.len() > 1 {
        return Err(failure(
            Vec::new(),
            "NORMALIZER",
            InvalidOutputType::ExtraText,
            None,
            "A resposta contém mais de um objeto JSON",
            false,
        ));
    }
    let Some((begin, end)) = spans.first().copied() else {
        return Err(failure(
            ...
            InvalidOutputType::ExtraText,
            None,
            "Nenhum objeto JSON inequívoco foi encontrado",
            false,
        ));
    };
    let candidate = &trimmed[begin..end];
    serde_json::from_str::<Value>(candidate).map_err(|_| {
        failure(
            Vec::new(),
            "NORMALIZER",
            InvalidOutputType::InvalidJson,
            None,
            "O objeto encontrado não é JSON válido",
            true,
        )
    })?;
...
fn required<'a>(
    ...
    field: &str,
    stages: &[DecisionValidationStage],
    normalized: bool,
    ...
    object.get(field).ok_or_else(|| {
        failure(
            stages.to_vec(),
            "SCHEMA_VALIDATOR",
            InvalidOutputType::MissingField,
            Some(field),
...
pub fn validate(
    ...
    let Some(object) = value.as_object() else {
        return Err(failure(
            ...
            None,
            "A raiz da resposta deve ser um objeto JSON",
            normalized,
        ));
    };
    ...
    if let Some(extra) = object.keys().find(|key| !allowed.contains(&key.as_str())) {
        return Err(failure(
            ...
            InvalidOutputType::SchemaTypeError,
            Some(extra),
            format!("Campo não permitido: {extra}"),
            normalized,
        ));
    }
    ...
    let Some(target) = target_value.as_f64() else {
        return Err(failure(
            ...
            "SCHEMA_VALIDATOR",
            InvalidOutputType::SchemaTypeError,
            Some("target_exposure_pct"),
            "target_exposure_pct deve ser number",
            normalized,
        ));
    };
    ...
    if reason.trim().is_empty() || reason.chars().count() > 240 {
        return Err(failure(
            ...
            InvalidOutputType::SchemaTypeError,
            Some("reason"),
            "reason deve conter entre 1 e 240 caracteres",
            normalized,
        ));
    }
    stages.push(pass("SCHEMA_VALIDATOR"));

    let action = LifecycleAction::from_str(action_name).map_err(|_| {
        failure(
            stages.clone(),
            "SEMANTIC_VALIDATOR",
            InvalidOutputType::InvalidAction,
            Some("action"),
            format!("Ação não permitida: {action_name}"),
            normalized,
        )
    })?;
    if !target.is_finite() || !(0.0..=100.0).contains(&target) {
        return Err(failure(
            stages,
            "SEMANTIC_VALIDATOR",
            InvalidOutputType::InvalidTargetExposure,
            Some("target_exposure_pct"),
            "target_exposure_pct deve estar entre 0 e 100",
            normalized,
        ));
    }
    if !confidence.is_finite() || !(0.0..=1.0).contains(&confidence) {
        return Err(failure(
            stages,
            "SEMANTIC_VALIDATOR",
            InvalidOutputType::InvalidConfidence,
            Some("confidence"),
    ...
    if !REASON_CODES.contains(&reason_code) {
        return Err(failure(
            stages,
            "SEMANTIC_VALIDATOR",
            InvalidOutputType::InvalidReasonCode,
            Some("reason_code"),
            format!("reason_code não permitido: {reason_code}"),
            normalized,
        ));
    }
    ...
    let position =
        market_positions::validate_decision(position_state, current_exposure_pct, action, target);
    if position.validation_code.is_some() {
        let code = match (position_state, action) {
            ...
            (PositionState::LongOpen | PositionState::LongReduced, LifecycleAction::StayFlat) => {
        ...
        let mut result = failure(
            stages,
            "POSITION_VALIDATOR",
            InvalidOutputType::InvalidPositionAction,
            Some("target_exposure_pct"),
            code,
            normalized,
        );
...
pub fn validate_v4_2(
    ...
    validate_intent(input, position_state, REASON_CODES)
}
...
pub fn validate_intent(
    ...
    let Some(confidence_pct) = confidence_value.as_f64() else {
        return Err(failure(
            ...
            InvalidOutputType::SchemaTypeError,
            Some("confidence_pct"),
            "confidence_pct deve ser number",
            normalized,
        ));
    };
    let Some(reason_code) = reason_code_value.as_str() else {
    ...
    let intent = market_positions::PositionIntent::from_str(intent_name).map_err(|_| {
        failure(
            ...
            Some("intent"),
            format!("Intent não permitido: {intent_name}"),
            normalized,
        )
    })?;
    if !confidence_pct.is_finite() || !(0.0..=100.0).contains(&confidence_pct) {
        return Err(failure(
            ...
            Some("confidence_pct"),
            "confidence_pct deve estar entre 0 e 100",
            normalized,
        ));
    }
    if !reason_codes.contains(&reason_code) {
    ...
    if !market_positions::intent_allowed(position_state, intent) {
        return Err(failure(
            stages,
            "POSITION_INTENT_VALIDATOR",
            InvalidOutputType::InvalidPositionIntent,
            Some("intent"),
            format!("{} não é permitido para o estado atual", intent.as_str()),
            normalized,
        ));
    }
...
mod tests {
    ...
    fn valid(action: &str, target: &str) -> String {
        format!(
            r#"{{"action":"{action}","target_exposure_pct":{target},"confidence":0.7,"reason_code":"INSUFFICIENT_SIGNAL","reason":"controlled"}}"#
        )
    }
    ...
    fn exact_and_wrapped_json_are_distinguished() {
        assert!(
            !validate(&valid("STAY_FLAT", "0"), PositionState::Flat, 0.0)
                .unwrap()
    ...
    fn malformed_empty_and_multiple_objects_are_classified() {
        assert_eq!(
            validate("", PositionState::Flat, 0.0)
                .unwrap_err()
                .error_type,
            InvalidOutputType::EmptyResponse
        );
        assert_eq!(
            validate("{ action: BUY }", PositionState::Flat, 0.0)
                .unwrap_err()
                .error_type,
            InvalidOutputType::InvalidJson
        );
    ...
    fn schema_reports_missing_types_and_extra_fields() {
        ...
        assert_eq!(
            validate(r#"{"action":"STAY_FLAT"}"#, PositionState::Flat, 0.0)
                .unwrap_err()
                .error_type,
            InvalidOutputType::MissingField
        );
    ...
    fn semantic_ranges_and_enums_are_strict() {
        assert_eq!(
            validate(&valid("SHORT", "0"), PositionState::Flat, 0.0)
                .unwrap_err()
                .error_type,
            InvalidOutputType::InvalidAction
        );
        assert_eq!(
            validate(&valid("STAY_FLAT", "-1"), PositionState::Flat, 0.0)
                .unwrap_err()
                .error_type,
            InvalidOutputType::InvalidTargetExposure
        );
        assert_eq!(
            validate(&valid("STAY_FLAT", "101"), PositionState::Flat, 0.0)
                .unwrap_err()
                .error_type,
            InvalidOutputType::InvalidTargetExposure
        );
        ...
        assert_eq!(
            validate(&invalid_confidence, PositionState::Flat, 0.0)
                .unwrap_err()
                .error_type,
            InvalidOutputType::InvalidConfidence
        );
        ...
        assert_eq!(
            validate(&negative_confidence, PositionState::Flat, 0.0)
                .unwrap_err()
                .error_type,
            InvalidOutputType::InvalidConfidence
        );
        ...
        assert_eq!(
            validate(&invalid_reason, PositionState::Flat, 0.0)
                .unwrap_err()
                .error_type,
            InvalidOutputType::InvalidReasonCode
        );
    ...
    fn position_semantics_cover_flat_and_long_states() {
        for action in ["EXIT_LONG", "REDUCE_LONG"] {
            assert_eq!(
                validate(&valid(action, "0"), PositionState::Flat, 0.0)
                    .unwrap_err()
                    .error_type,
                InvalidOutputType::InvalidPositionAction
            );
    ...
    fn schema_uses_the_canonical_reason_code_registry() {
        assert_eq!(
            output_schema()["properties"]["reason_code"]["enum"]
                .as_array()
                .unwrap()
                .len(),
            REASON_CODES.len()
    ...
    fn valid_v42(intent: &str, confidence: &str) -> String {
        format!(
            r#"{{"intent":"{intent}","confidence_pct":{confidence},"reason_code":"INSUFFICIENT_SIGNAL","reason":"controlled"}}"#
        )
    }
    ...
    fn v42_accepts_percentage_confidence_and_normalizes_it() {
        for confidence in ["0", "83", "100"] {
            let parsed =
                validate_v4_2(&valid_v42("ENTER", confidence), PositionState::Flat).unwrap();
            assert_eq!(
                parsed.confidence,
                confidence.parse::<f64>().unwrap() / 100.0
            );
        }
        for confidence in ["-1", "101"] {
            assert_eq!(
                validate_v4_2(&valid_v42("ENTER", confidence), PositionState::Flat)
                    .unwrap_err()
                    .error_type,
                InvalidOutputType::InvalidConfidencePct
            );
    ...
    fn v42_validates_intent_against_position_without_accepting_targets() {
        ...
        for intent in ["REDUCE", "EXIT"] {
            assert_eq!(
                validate_v4_2(&valid_v42(intent, "80"), PositionState::Flat)
                    .unwrap_err()
                    .error_type,
                InvalidOutputType::InvalidPositionIntent
            );
        ...
        assert_eq!(
            validate_v4_2(&valid_v42("BUY", "80"), PositionState::Flat)
                .unwrap_err()
                .error_type,
            InvalidOutputType::InvalidIntent
        );
