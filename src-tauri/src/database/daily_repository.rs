...
pub fn complete_task(connection: &Connection, id: &str) -> Result<Task, String> {
    ...
    if changed == 0 { return Err("Tarefa não encontrada".into()); }
...
pub fn delete_task(connection: &Connection, id: &str) -> Result<(), String> {
    connection.execute("DELETE FROM tasks WHERE id=?1", [id]).map_err(err)?;
    Ok(())
}
...
pub fn save_note(connection: &Connection, input: &NoteInput) -> Result<Note, String> {
    validate_note(input)?;
    let title = input.title.as_deref().map(str::trim).filter(|value| !value.is_empty());
    connection.execute(
...
pub fn archive_note(connection: &Connection, id: &str) -> Result<Note, String> {
    let changed = connection.execute("UPDATE notes SET status='archived',updated_at=CURRENT_TIMESTAMP WHERE id=?1", [id]).map_err(err)?;
    if changed == 0 { return Err("Nota não encontrada".into()); }
...
pub fn delete_note(connection: &Connection, id: &str) -> Result<(), String> {
    connection.execute("DELETE FROM notes WHERE id=?1", [id]).map_err(err)?;
    Ok(())
}
...
fn query_tasks<P: rusqlite::Params>(connection: &Connection, sql: &str, params: P) -> Result<Vec<Task>, String> {
    let mut statement = connection.prepare(sql).map_err(err)?;
...
fn map_task(row: &rusqlite::Row<'_>) -> rusqlite::Result<Task> {
    Ok(Task { id: row.get(0)?, title: row.get(1)?, description: row.get(2)?, status: row.get(3)?, priority: row.get(4)?, due_date: row.get(5)?, project_id: row.get(6)?, knowledge_area_id: row.get(7)?, created_at: row.get(8)?, updated_at: row.get(9)?, completed_at: row.get(10)? })
}

fn map_note(row: &rusqlite::Row<'_>) -> rusqlite::Result<Note> {
    Ok(Note { id: row.get(0)?, title: row.get(1)?, content: row.get(2)?, status: row.get(3)?, project_id: row.get(4)?, knowledge_area_id: row.get(5)?, created_at: row.get(6)?, updated_at: row.get(7)? })
}

fn validate_task(input: &TaskInput) -> Result<(), String> {
    validate_id(&input.id)?;
    if input.title.trim().is_empty() { return Err("Título da tarefa é obrigatório".into()); }
    if !["inbox","pending","in_progress","completed","cancelled"].contains(&input.status.as_str()) { return Err("Status da tarefa inválido".into()); }
    if !["low","medium","high","critical"].contains(&input.priority.as_str()) { return Err("Prioridade inválida".into()); }
    if let Some(date) = &input.due_date { validate_date(date)?; }
    Ok(())
}

fn validate_note(input: &NoteInput) -> Result<(), String> {
    validate_id(&input.id)?;
    if input.content.trim().is_empty() { return Err("Conteúdo da nota é obrigatório".into()); }
    if !["active","archived"].contains(&input.status.as_str()) { return Err("Status da nota inválido".into()); }
    Ok(())
}

fn validate_id(value: &str) -> Result<(), String> {
    if !value.is_empty() && value.chars().all(|character| character.is_ascii_alphanumeric() || character == '-' || character == '_') { Ok(()) } else { Err("ID inválido".into()) }
}

fn validate_date(value: &str) -> Result<(), String> {
    let parts = value.split('-').map(|part| part.parse::<u32>()).collect::<Result<Vec<_>, _>>().map_err(|_| "Data inválida; use AAAA-MM-DD".to_string())?;
    if parts.len() != 3 || value.len() != 10 { return Err("Data inválida; use AAAA-MM-DD".into()); }
    let (year, month, day) = (parts[0], parts[1], parts[2]);
    let leap = year % 4 == 0 && (year % 100 != 0 || year % 400 == 0);
    let max_day = match month { 1|3|5|7|8|10|12 => 31, 4|6|9|11 => 30, 2 if leap => 29, 2 => 28, _ => 0 };
    if year >= 1900 && day >= 1 && day <= max_day { Ok(()) } else { Err("Data inválida; use AAAA-MM-DD".into()) }
}
...
mod tests {
    ...
    fn task(id: &str) -> TaskInput { TaskInput { id: id.into(), title: "Revisar GeneScope".into(), description: "Comparar variantes".into(), status: "inbox".into(), priority: "high".into(), due_date: Some("2026-08-31".into()), project_id: Some("genescope".into()), knowledge_area_id: Some("genetics".into()) } }
    ...
    fn task_crud_completion_relations_and_filters() {
        ...
        assert_eq!(list_today_tasks(&connection, "2026-08-31").unwrap().len(), 1);
        let completed = complete_task(&connection, "task-1").unwrap();
        assert_eq!(completed.status, "completed");
        assert!(completed.completed_at.is_some());
        let mut reopened = task("task-1"); reopened.status = "in_progress".into(); reopened.title = "Revisar lógica".into();
    ...
    fn upcoming_and_counters_follow_local_date() {
        let connection = database();
        let mut future = task("future"); future.status = "pending".into(); future.due_date = Some("2026-09-01".into()); save_task(&connection, &future).unwrap();
        let mut overdue = task("overdue"); overdue.status = "pending".into(); overdue.due_date = Some("2026-08-30".into()); save_task(&connection, &overdue).unwrap();
        let mut current = task("current"); current.status = "in_progress".into(); current.due_date = None; save_task(&connection, &current).unwrap();
        assert_eq!(list_upcoming_tasks(&connection, "2026-08-31").unwrap().len(), 1);
        assert_eq!(list_today_tasks(&connection, "2026-08-31").unwrap().len(), 2);
    ...
    fn note_crud_and_archive() {
        let connection = database();
        let input = NoteInput { id: "note-1".into(), title: Some("MQTT".into()), content: "Investigar sensores".into(), status: "active".into(), project_id: Some("arccore".into()), knowledge_area_id: Some("iot".into()) };
        ...
        assert_eq!(archive_note(&connection, "note-1").unwrap().status, "archived");
    ...
    fn task_and_note_survive_file_reopen() {
        ...
        { let mut connection = Connection::open(&path).unwrap(); database::initialize(&mut connection).unwrap(); save_task(&connection, &task("persistent-task")).unwrap(); save_note(&connection, &NoteInput { id: "persistent-note".into(), title: None, content: "Nota persistente".into(), status: "active".into(), project_id: None, knowledge_area_id: None }).unwrap(); }
        { let mut connection = Connection::open(&path).unwrap(); database::initialize(&mut connection).unwrap(); assert!(get_task(&connection, "persistent-task").unwrap().is_some()); assert!(get_note(&connection, "persistent-note").unwrap().is_some()); }
