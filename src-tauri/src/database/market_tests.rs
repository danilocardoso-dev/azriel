...
impl market_ai::MarketAiProvider for FakeMarketAiProvider {
    fn complete(
        ...
        self.prompts.lock().unwrap().push(prompt.snapshot_json);
        Ok(market_ai::MarketAiProviderResponse{content:r#"{"action":"BUY","desired_position_pct":90,"confidence":0.99,"reason":"strong signal"}"#.into(),model:"fake:market".into(),input_tokens:Some(20),output_tokens:Some(10)})
    }
}

fn fake_ai_config() -> market_ai::MarketAiConfig {
    market_ai::MarketAiConfig {
        agent_id: market_ai::AI_AGENT_ID.into(),
        provider: "ollama".into(),
        model: "fake:market".into(),
        prompt_version: market_ai::PROMPT_VERSION.into(),
        temperature: 0.1,
        decision_interval: 5,
        timeout_ms: 5000,
        max_retries: 1,
...
impl market_ai::MarketAiProvider for CalibratedFakeProvider {
    fn complete(
        ...
        let positioned = snapshot
            ...
            .unwrap_or(false);
        let content = if positioned {
            r#"{"action":"SELL","desired_position_pct":0,"confidence":0.72,"reason":"trend evidence deteriorated"}"#
        } else {
            r#"{"action":"BUY","desired_position_pct":80,"confidence":0.68,"reason":"trend and momentum evidence converge"}"#
        };
        self.prompts.lock().unwrap().push(prompt);
        Ok(market_ai::MarketAiProviderResponse {
            content: content.into(),
            model: "fake:qwen-v2".into(),
...
fn fake_ai_v2_config() -> market_ai::MarketAiConfig {
    market_ai::MarketAiConfig {
        agent_id: market_ai::AI_AGENT_V2_ID.into(),
        provider: "ollama".into(),
        model: "fake:qwen-v2".into(),
        prompt_version: market_ai::PROMPT_V2_VERSION.into(),
        temperature: 0.1,
        decision_interval: 5,
        timeout_ms: 5000,
        max_retries: 1,
...
struct SignalAwareFakeProvider {
    prompts: Mutex<Vec<market_ai::MarketAiPrompt>>,
}
impl market_ai::MarketAiProvider for SignalAwareFakeProvider {
    fn complete(
        &self,
        prompt: market_ai::MarketAiPrompt,
        _: &market_ai::MarketAiConfig,
        ...
        let snapshot: serde_json::Value = serde_json::from_str(&prompt.snapshot_json).unwrap();
        let bias = snapshot
            .pointer("/signals/directionalBias")
        ...
        let positioned = snapshot
            ...
            .and_then(|value| value.as_bool())
            .unwrap_or(false);
        let content = match (bias, positioned) {
            ("BULLISH", false) => {
                r#"{"action":"BUY","desired_position_pct":40,"confidence":0.74,"reason_code":"ALIGNED_BULLISH_SIGNAL","reason":"aligned bullish evidence"}"#
            }
            ("BEARISH", true) => {
                r#"{"action":"SELL","desired_position_pct":0,"confidence":0.71,"reason_code":"EXIT_SIGNAL","reason":"bearish exit evidence"}"#
            }
            _ => {
                r#"{"action":"HOLD","desired_position_pct":0,"confidence":0.46,"reason_code":"INSUFFICIENT_SIGNAL","reason":"signal not actionable"}"#
            }
        };
        self.prompts.lock().unwrap().push(prompt);
        Ok(market_ai::MarketAiProviderResponse {
            content: content.into(),
            model: "fake:qwen-v3".into(),
            input_tokens: Some(40),
            output_tokens: Some(14),
        })
    }
}
fn fake_ai_v3_config() -> market_ai::MarketAiConfig {
    market_ai::MarketAiConfig {
        agent_id: market_ai::AI_AGENT_V3_ID.into(),
        provider: "ollama".into(),
        model: "fake:qwen-v3".into(),
        prompt_version: market_ai::PROMPT_V3_VERSION.into(),
        temperature: 0.1,
        decision_interval: 5,
        timeout_ms: 5000,
        max_retries: 1,
...
struct PositionAwareFakeProvider {
    calls: Mutex<usize>,
    prompts: Mutex<Vec<market_ai::MarketAiPrompt>>,
}
impl market_ai::MarketAiProvider for PositionAwareFakeProvider {
    fn complete(
        &self,
        prompt: market_ai::MarketAiPrompt,
        _: &market_ai::MarketAiConfig,
        ...
        let (action, target, code) = match *calls {
            0 => ("ENTER_LONG", 30, "ALIGNED_BULLISH_SIGNAL"),
            1 => ("HOLD_POSITION", 30, "POSITION_ALREADY_OPTIMAL"),
            2 => ("INCREASE_LONG", 45, "ALIGNED_BULLISH_SIGNAL"),
            3 => ("REDUCE_LONG", 20, "REDUCE_RISK"),
            4 => ("EXIT_LONG", 0, "EXIT_SIGNAL"),
            _ => ("STAY_FLAT", 0, "INSUFFICIENT_SIGNAL"),
        };
        *calls += 1;
        drop(calls);
        self.prompts.lock().unwrap().push(prompt);
        Ok(market_ai::MarketAiProviderResponse {
            content: format!(
                r#"{{"action":"{action}","target_exposure_pct":{target},"confidence":0.75,"reason_code":"{code}","reason":"position lifecycle test"}}"#
            ),
            model: "fake:qwen-v4".into(),
            input_tokens: Some(50),
            output_tokens: Some(16),
        })
    }
}
fn fake_ai_v4_config() -> market_ai::MarketAiConfig {
    market_ai::MarketAiConfig {
        agent_id: market_ai::AI_AGENT_V4_ID.into(),
        provider: "ollama".into(),
        model: "fake:qwen-v4".into(),
        prompt_version: market_ai::PROMPT_V4_VERSION.into(),
        temperature: 0.1,
        decision_interval: 5,
        timeout_ms: 5000,
        max_retries: 1,
...
fn fixture_csv() -> String {
    ...
    for day in 1..=30 {
        ...
        output.push_str(&format!(
            "2026-01-{day:02}T00:00:00Z,{open},{},{},{close},1000\n",
            open + 2.0,
            open - 2.0
        ));
    }
...
fn complete_market_flow_is_persistent_auditable_and_reproducible() {
    ...
    let import = || ImportMarketDatasetInput {
    ...
    let input = MarketExperimentInput {
        ...
        agent_ids: vec![
            ...
            "buy-hold".into(),
            "simple-trend".into(),
            "simple-momentum".into(),
            "random-controlled".into(),
        ],
    ...
    let cash_behavior = first
        .observatory
        .behavior
        .iter()
        .find(|metric| metric.agent_id == "cash")
        .unwrap();
    ...
    assert_eq!(
        connection
            .query_row(
                "SELECT COUNT(*) FROM market_behavior_metrics WHERE experiment_id=?1",
                [&first.experiment.id],
                |row| row.get::<_, i64>(0)
            )
            .unwrap(),
    ...
    assert_eq!(
        first
            .metrics
            .iter()
            .map(|m| (m.agent_id.as_str(), m.final_equity, m.trade_count))
            .collect::<Vec<_>>(),
        second
            .metrics
            .iter()
    ...
    assert!(first
        .decisions
        .iter()
        .any(|decision| decision.action == "HOLD"));
    assert!(first
        .decisions
        .iter()
        .any(|decision| decision.execution_price.is_some()));
    assert_eq!(
...
fn six_agents_are_rejected_before_an_experiment_is_created() {
    ...
    assert_eq!(
        connection
            .query_row("SELECT COUNT(*) FROM market_experiments", [], |row| row
                .get::<_, i64>(0))
            .unwrap(),
        0
    );
...
fn cohort_requires_at_least_three_agents() {
    ...
    let input = MarketExperimentInput {
        ...
        dataset_id: "missing".into(),
        risk_profile_id: "balanced-v1".into(),
        agent_ids: vec!["cash".into(), "buy-hold".into()],
        initial_capital: 100.0,
        random_seed: 1,
        fee_pct: 0.0,
        slippage_pct: 0.0,
...
fn cohorts_support_three_four_and_five_agents_with_equal_starting_capital() {
    ...
    for size in 3..=5 {
        let result = market_repository::run_experiment(
            &mut connection,
            &MarketExperimentInput {
                name: format!("Cohort {size}"),
                dataset_id: dataset.id.clone(),
                risk_profile_id: "balanced-v1".into(),
                agent_ids: registry[..size].iter().map(|id| (*id).into()).collect(),
        ...
        assert!(result
            .equity
            .iter()
            .filter(|point| point.timestamp == "2026-01-01T00:00:00Z")
            .all(|point| point.equity == 50.0));
        assert!(result
            .observatory
            .correlations
            .iter()
            .filter(|cell| cell.agent_a_id == cell.agent_b_id)
            .all(|cell| cell.equity_return_correlation == Some(1.0)
                && cell.decision_similarity == 1.0));
    }
    let _ = fs::remove_file(path);
}
...
fn scientific_validation_is_persistent_frozen_and_reproducible() {
    ...
    assert_eq!(
        first.audit.validation_engine_version,
    ...
    assert!(first
        .regime_metrics
        .iter()
        .any(|metric| metric.low_sample_size));
    assert!(first
        .metrics
        .iter()
        .flat_map(|metric| [metric.sharpe, metric.sortino, metric.calmar])
        .flatten()
        .all(|value| value.is_finite()));
    assert!(first
        .metrics
        .iter()
        .filter(|metric| metric.agent_id == "cash")
        .all(|metric| metric.total_return_pct == 0.0));
    assert_eq!(
        first
            .reports
            .iter()
            .map(|report| (
                ...
                report.out_of_sample_return_pct,
                report.positive_window_ratio_pct,
                report.robustness_status.as_str()
            ))
            .collect::<Vec<_>>(),
        second
            .reports
...
fn scientific_validation_persists_failed_status_after_runtime_error() {
    ...
    let (status, persisted_error): (String, String) = connection
        .query_row(
...
fn scientific_regime_fixture_imports_all_candles_in_temporal_order() {
    ...
    fs::write(
        &path,
        include_str!("../../../Docs/market-lab/fixtures/deterministic-regimes-160-candles.csv"),
    )
    .unwrap();
    let mut connection = database();
    let dataset = market_repository::import_dataset(
...
fn ai_agent_uses_structured_snapshots_risk_and_persistent_runtime() {
    ...
    let provider = FakeMarketAiProvider {
    ...
    assert!(result
        ...
        .iter()
        .filter(|point| point.timestamp == "2026-01-01T00:00:00Z")
        .all(|point| point.equity == 1_000.0));
    ...
    assert_eq!(
        decisions
            .iter()
            .filter(|decision| decision.call_status == "NO_LLM_CALL")
            .count(),
        24
    );
    assert!(decisions
        .iter()
        .filter(|decision| decision.call_status == "VALID")
        .all(|decision| decision.approved_position_pct.unwrap_or(0.0) <= 50.0));
    ...
    assert!(decisions
        .iter()
        .filter(|decision| decision.call_status == "VALID")
        .all(|decision| decision.input_snapshot.is_some()));
    assert!(decisions
        .iter()
        .filter(|decision| decision.call_status == "NO_LLM_CALL")
        .all(|decision| decision.input_snapshot.is_none()));
    drop(prompts);
    let validation_input = MarketValidationInput {
        name: "AI-VAL-001".into(),
        dataset_id: dataset.id,
        risk_profile_id: "balanced-v1".into(),
    ...
    let validation = market_validation::run_with_provider(
        &mut connection,
        &validation_input,
        &provider,
        &fake_ai_config(),
    ...
    assert!(validation
        .reports
        .iter()
        .any(|report| report.agent_id == market_ai::AI_AGENT_ID));
    let validation_runtime =
        market_repository::list_validation_ai_runtime(&connection, &validation.validation.id)
            .unwrap();
    ...
    assert!(validation
        .audit
        .frozen_config_json
        .contains(market_ai::PROMPT_VERSION));
    let _ = fs::remove_file(path);
}
...
fn ai_v2_persists_rich_context_distribution_forward_returns_and_ab_metadata() {
    ...
    let dataset = market_repository::import_dataset(
        ...
        &ImportMarketDatasetInput {
            path: path.to_string_lossy().into_owned(),
            name: "AI calibration fixture".into(),
            asset: "AAPL".into(),
            timeframe: "1D".into(),
    ...
    let provider = CalibratedFakeProvider {
        prompts: Mutex::new(Vec::new()),
    };
    let mut v2_agents = base_agents.clone();
    v2_agents.push(market_ai::AI_AGENT_V2_ID.into());
    let v2_input = MarketExperimentInput {
        name: "AI-CALIBRATION-001".into(),
        dataset_id: dataset.id.clone(),
        risk_profile_id: "balanced-v1".into(),
        agent_ids: v2_agents,
        initial_capital: 10_000.0,
    ...
    let v2 = market_repository::run_experiment_with_provider(
        &mut connection,
        &v2_input,
        &provider,
        &fake_ai_v2_config(),
    ...
    .unwrap();
    let runtime = market_repository::list_ai_runtime(&connection, &v2.experiment.id)
        .unwrap()
        .remove(0);
    assert_eq!(runtime.call_count, 6);
    assert_eq!(runtime.successful_call_count, 6);
    assert_eq!(
        runtime.buy_count + runtime.sell_count + runtime.llm_hold_count,
        6
    );
    ...
    let called: Vec<_> = decisions
        .iter()
        .filter(|decision| decision.call_status == "VALID")
        .collect();
    assert_eq!(called.len(), 6);
    assert!(called.iter().all(|decision| decision
        .input_snapshot
        .as_ref()
        .and_then(|snapshot| snapshot.get("market"))
        .is_some()));
    assert!(called.iter().all(|decision| !decision
        .input_snapshot
        .as_ref()
        .unwrap()
        .to_string()
        .contains("forward_return")));
    assert!(called
        .iter()
        .any(|decision| decision.forward_return_1.is_some()));
    assert!(called
        .iter()
        .any(|decision| decision.forward_return_10.is_none()));
    assert!(called
        .iter()
    ...
    assert!(prompts.iter().all(|prompt| prompt
        .system
        .contains("Do not require every indicator to agree")));
    assert!(prompts
        .iter()
        .all(|prompt| !prompt.snapshot_json.contains("forward_return")));
    drop(prompts);

    let v1_provider = FakeMarketAiProvider {
        prompts: Mutex::new(Vec::new()),
    };
    let mut v1_agents = base_agents;
    v1_agents.push(market_ai::AI_AGENT_ID.into());
    let v1_input = MarketExperimentInput {
        name: "AI-CALIBRATION-001 / V1".into(),
        agent_ids: v1_agents,
        dataset_id: dataset.id.clone(),
        ..v2_input.clone()
    };
    market_repository::run_experiment_with_provider(
        &mut connection,
        &v1_input,
        &v1_provider,
        &fake_ai_config(),
    ...
    assert_eq!(
        comparisons[0].decision_interval,
        comparisons[1].decision_interval
    );
    assert!(comparisons
        .iter()
        .any(|entry| entry.prompt_version == market_ai::PROMPT_VERSION));
    assert!(comparisons
        .iter()
        .any(|entry| entry.prompt_version == market_ai::PROMPT_V2_VERSION));

    let invalid = MarketExperimentInput {
        name: "invalid dual AI".into(),
        agent_ids: vec![
            "cash".into(),
            market_ai::AI_AGENT_ID.into(),
            market_ai::AI_AGENT_V2_ID.into(),
        ],
        dataset_id: dataset.id,
        risk_profile_id: "balanced-v1".into(),
        initial_capital: 10_000.0,
    ...
    assert!(market_repository::validate_input(&connection, &invalid)
        .unwrap_err()
        .contains("no máximo um AI Agent"));
    let _ = fs::remove_file(path);
}
...
fn ai_v3_persists_signal_trace_reason_codes_diagnostics_and_frozen_config() {
    let suffix = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let path = std::env::temp_dir().join(format!("azriel-market-ai-v3-{suffix}.csv"));
    fs::write(&path, fixture_csv()).unwrap();
    let mut connection = database();
    let dataset = market_repository::import_dataset(
        &mut connection,
    ...
    let provider = SignalAwareFakeProvider {
        prompts: Mutex::new(Vec::new()),
    };
    let result = market_repository::run_experiment_with_provider(
        &mut connection,
    ...
    assert!(valid.iter().all(|item| item.reason_code.is_some()));
    assert!(valid.iter().all(|item| item
        .input_snapshot
        .as_ref()
    ...
    let diagnostics = market_repository::get_signal_diagnostics(&connection, &result.experiment.id)
        .unwrap()
        .unwrap();
    assert_eq!(diagnostics.signal_engine_version, "SIGNAL_ENGINE_V1");
    assert_eq!(
        diagnostics
            .signal_action_matrix
            .iter()
            .map(|row| row.buy_count + row.sell_count + row.hold_count)
            .sum::<usize>(),
        6
    );
    ...
    let frozen: String = connection
        ...
        .unwrap();
    assert!(frozen.contains("SIGNAL_ENGINE_V1"));
    assert!(frozen.contains("SIGNAL_CONFIG_V1"));
    let prompts = provider.prompts.lock().unwrap();
    assert!(prompts
        .iter()
        .all(|prompt| prompt.system.contains("SIGNAL SCORES ARE PRE-COMPUTED")));
    drop(prompts);
    let invalid = MarketExperimentInput {
        agent_ids: vec![
            "cash".into(),
...
#[test]
fn ai_v4_manages_and_persists_a_complete_position_lifecycle() {
    let suffix = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
        .as_nanos();
    let path = std::env::temp_dir().join(format!("azriel-market-ai-v4-{suffix}.csv"));
    fs::write(&path, fixture_csv()).unwrap();
    let mut connection = database();
    let dataset = market_repository::import_dataset(
        &mut connection,
    ...
    .unwrap();
    let input = MarketExperimentInput {
        name: "AI-POSITION-LIFECYCLE".into(),
        dataset_id: dataset.id,
        risk_profile_id: "balanced-v1".into(),
        agent_ids: vec![
            "cash".into(),
    ...
    let provider = PositionAwareFakeProvider {
        calls: Mutex::new(0),
        prompts: Mutex::new(Vec::new()),
    };
    let result = market_repository::run_experiment_with_provider(
    ...
    .unwrap();
    let report = &result.position_lifecycle;
    let lifecycles = report
        .lifecycles
        .iter()
        .filter(|item| item.agent_id == market_ai::AI_AGENT_V4_ID)
        .collect::<Vec<_>>();
    assert_eq!(lifecycles.len(), 1);
    assert_eq!(lifecycles[0].status, "CLOSED");
    let events = report
        .events
        .iter()
        .filter(|item| item.lifecycle_id == lifecycles[0].id)
        .collect::<Vec<_>>();
    for action in [
        "ENTER_LONG",
        "HOLD_POSITION",
    ...
    assert!(events.len() > lifecycles.len());
    let metric = report
        .metrics
        .iter()
        .find(|item| item.agent_id == market_ai::AI_AGENT_V4_ID)
        .unwrap();
    assert_eq!(metric.closed_trades, 1);
    assert_eq!(metric.engine_version, "POSITION_ENGINE_V1");
    assert!(lifecycles[0].mfe_pct.is_finite() && lifecycles[0].mae_pct.is_finite());
    let decisions =
        market_repository::list_ai_decisions(&connection, &result.experiment.id).unwrap();
    assert!(decisions
        .iter()
        ...
        .all(|item| item
            .input_snapshot
            .as_ref()
            .is_some_and(|snapshot| snapshot.get("position").is_some()
                && !snapshot.to_string().contains("forward_return"))));
    assert!(
        market_repository::get_signal_diagnostics(&connection, &result.experiment.id)
            .unwrap()
            .is_some()
    );
    let frozen: String = connection
        ...
        .unwrap();
    assert!(frozen.contains("POSITION_ENGINE_V1"));
    assert!(provider
        .prompts
        .lock()
        .unwrap()
