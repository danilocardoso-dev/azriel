...
pub fn ensure_research_seed(connection: &mut Connection) -> Result<(), String> {
    let seeded = connection
        .query_row(
            "SELECT EXISTS(SELECT 1 FROM _azriel_seeds WHERE key='v0.8.2-research')",
            [],
...
pub fn list_baselines(connection: &Connection) -> Result<Vec<KnowledgeBaseline>, String> {
    ...
    let rows = statement
        .query_map([], |row| {
            Ok(KnowledgeBaseline {
                knowledge_area_id: row.get(0)?,
                coverage: row.get(1)?,
                depth: row.get(2)?,
...
fn list_activities(
    ...
    for activity in &mut activities {
        ...
        let values = relations
            ...
            .map_err(err)?;
        activity.primary_knowledge_node_id = values
            .iter()
            .find(|(_, role)| role == "primary")
            .map(|(id, _)| id.clone());
        activity.secondary_knowledge_node_ids = values
            .into_iter()
            .filter(|(_, role)| role == "secondary")
            .map(|(id, _)| id)
            .collect();
    }
    Ok(activities)
...
fn list_topics(connection: &Connection, stage_id: &str) -> Result<Vec<RoadmapTopic>, String> {
    ...
    for topic in &mut topics {
        ...
        topic.prerequisite_topic_ids = prerequisites
            .query_map([&topic.id], |row| row.get(0))
            .map_err(err)?
            .collect::<Result<Vec<_>, _>>()
            .map_err(err)?;
...
fn list_stages(connection: &Connection, roadmap_id: &str) -> Result<Vec<RoadmapStage>, String> {
    ...
    for stage in &mut stages {
        stage.topics = list_topics(connection, &stage.id)?;
    }
    Ok(stages)
}
...
pub fn list_roadmaps(connection: &Connection) -> Result<Vec<StudyRoadmap>, String> {
    ...
    for row in rows {
        ...
        let total_activities = stages
            .iter()
            .flat_map(|stage| &stage.topics)
            .map(|topic| topic.activities.len() as i64)
            .sum();
        let completed_activities = stages
            .iter()
            .flat_map(|stage| &stage.topics)
            .flat_map(|topic| &topic.activities)
            .filter(|activity| activity.status == "completed")
            .count() as i64;
        let progress = if total_activities == 0 {
            0
        } else {
            completed_activities * 100 / total_activities
        };
        result.push(StudyRoadmap {
            id,
            name,
            description,
            status,
...
pub fn save_roadmap(
    ...
    required(&input.id, "o ID do roadmap")?;
    required(&input.name, "o nome do roadmap")?;
    if !["planned", "active", "paused", "completed"].contains(&input.status.as_str()) {
    ...
    for (stage_index, stage) in input.stages.iter().enumerate() {
        required(&stage.id, "o ID da etapa")?;
        required(&stage.name, "o nome da etapa")?;
        if stage.order != stage_index as i64 + 1 || !ids.insert(stage.id.clone()) {
            return Err("Etapas devem possuir IDs únicos e ordem contínua".into());
        }
        for (topic_index, topic) in stage.topics.iter().enumerate() {
            required(&topic.id, "o ID do tópico")?;
            required(&topic.name, "o nome do tópico")?;
            if topic.order != topic_index as i64 + 1 || !ids.insert(topic.id.clone()) {
                return Err("Tópicos devem possuir IDs únicos e ordem contínua".into());
            }
            if ![
                "NOT_STARTED",
                "EXPOSED",
            ...
            .contains(&topic.state.as_str())
            {
                return Err("Estado de tópico inválido".into());
            }
            if let Some(knowledge_id) = &topic.knowledge_node_id {
                ...
                if !exists {
                    return Err(format!(
                        "Conhecimento não encontrado no tópico {}",
                        topic.name
                    ));
                }
            }
            for prerequisite_id in &topic.prerequisite_topic_ids {
                if prerequisite_id == &topic.id {
                    return Err("Um tópico não pode ser pré-requisito de si mesmo".into());
                }
                if !topic_ids.contains(prerequisite_id) {
                    return Err(format!(
                        "Pré-requisito não encontrado no tópico {}",
                        topic.name
                    ));
                }
            }
            for (activity_index, activity) in topic.activities.iter().enumerate() {
                ...
                if activity.order != activity_index as i64 + 1 || !ids.insert(activity.id.clone()) {
                    return Err("Atividades devem possuir IDs únicos e ordem contínua".into());
                }
                if ![
                    "READING",
                    "LESSON",
                ...
                let primary = activity
                    ...
                    .as_ref()
                    .or(topic.knowledge_node_id.as_ref())
                    .ok_or_else(|| {
                        format!(
                            "A atividade {} exige um conhecimento primário",
                            activity.title
                        )
                    })?;
                let mut relation_ids = HashSet::from([primary.clone()]);
                for knowledge_id in &activity.secondary_knowledge_node_ids {
                    if !relation_ids.insert(knowledge_id.clone()) {
                        return Err("Conhecimentos relacionados não podem ser repetidos".into());
                    }
                }
                for knowledge_id in relation_ids {
                    let exists = connection
                        ...
                        .map_err(err)?;
                    if !exists {
                        return Err("Conhecimento relacionado não encontrado".into());
                    }
                }
            }
        }
    ...
    let old_relations: HashMap<String, HashSet<String>> = {
        ...
        for (activity, value) in rows {
            relations.entry(activity).or_default().insert(value);
        }
        relations
    };
    let new_ids: HashSet<String> = input
        .stages
        .iter()
        .flat_map(|stage| &stage.topics)
        .flat_map(|topic| &topic.activities)
        ...
        .collect();
    for (old_id, (old_status, _)) in &old_statuses {
        if !new_ids.contains(old_id) && old_status == "completed" {
            return Err("Reabra atividades concluídas antes de removê-las".into());
        }
    }
        ...
        .flat_map(|topic| &topic.activities)
    {
        if let Some((old_status, old_type)) = old_statuses.get(&activity.id) {
            ...
            {
                return Err(
                    "Reabra a atividade concluída antes de alterar seu tipo de evidência".into(),
                );
            }
        }
    }
    for topic in input.stages.iter().flat_map(|stage| &stage.topics) {
        for activity in &topic.activities {
            if old_statuses
                .get(&activity.id)
                .is_some_and(|value| value.0 == "completed")
                && activity.status == "completed"
            {
                let primary = activity
                    .primary_knowledge_node_id
                    ...
                    .or(topic.knowledge_node_id.as_ref())
                    .expect("validated");
                let mut current = HashSet::from([format!("primary:{primary}")]);
                current.extend(
                    activity
                ...
                {
                    return Err("Reabra a atividade concluída antes de alterar seus conhecimentos relacionados".into());
                }
            }
        }
    }
    let previous_events: HashSet<String> = learning_engine::list_events(connection)?
        .into_iter()
        .map(|event| event.id)
        .collect();
    let transaction = connection.transaction().map_err(err)?;
    ...
    transaction
        .execute(
            "DELETE FROM roadmap_stages WHERE roadmap_id=?1",
            [&input.id],
        )
        .map_err(err)?;
    for stage in &input.stages {
        ...
        for topic in &stage.topics {
            ...
            for activity in &topic.activities {
                let completed_at = if activity.status == "completed" {
                    activity
                        .completed_at
                        .clone()
                        .or_else(|| Some("CURRENT_TIMESTAMP".into()))
                } else {
                    None
                };
                ...
                for secondary in &activity.secondary_knowledge_node_ids {
                    transaction.execute("INSERT INTO activity_knowledge_nodes(activity_id,knowledge_node_id,role) VALUES (?1,?2,'secondary')",params![activity.id,secondary]).map_err(err)?;
                }
            }
        }
    }
        ...
        .flat_map(|topic| &topic.activities)
    {
        let old = old_statuses
            ...
            .unwrap_or("pending");
        if old != "completed" && activity.status == "completed" {
            learning_engine::complete_activity(&transaction, &activity.id)?;
        }
        if old == "completed" && activity.status != "completed" {
            learning_engine::reopen_activity(&transaction, &activity.id)?;
        }
    }
    let mut result =
        learning_engine::recalculate(&transaction, "Learning Engine: atividade de roadmap")?;
    result.created_events = learning_engine::list_events(&transaction)?
        .into_iter()
        .filter(|event| !previous_events.contains(&event.id))
        .collect();
    transaction.commit().map_err(err)?;
    Ok(result)
}
...
pub fn update_activity_status(
    ...
    if !["pending", "in_progress", "completed"].contains(&input.status.as_str()) {
        return Err("Status de atividade inválido".into());
    }
    let current_status = connection
        .query_row(
            "SELECT status FROM roadmap_activities WHERE id=?1",
            [&input.activity_id],
            |row| row.get::<_, String>(0),
        )
        .optional()
        .map_err(err)?
        .ok_or_else(|| "Atividade não encontrada".to_string())?;
    if current_status == input.status {
        let integration = connection
            ...
            .map_err(err)?;
        return Ok(RoadmapSaveResult {
            roadmaps: list_roadmaps(connection)?,
    ...
    if current_status != "completed" && input.status == "completed" {
        learning_engine::complete_activity(&transaction, &input.activity_id)?;
    }
    if current_status == "completed" && input.status != "completed" {
        learning_engine::reopen_activity(&transaction, &input.activity_id)?;
    }
    let mut learning = learning_engine::recalculate(
        &transaction,
        "Learning Engine: transição de atividade no roadmap",
    )?;
    learning.created_events = learning_engine::list_events(&transaction)?
        .into_iter()
        .filter(|event| !previous_events.contains(&event.id))
        .collect();
    transaction.commit().map_err(err)?;
...
pub fn delete_roadmap(connection: &Connection, id: &str) -> Result<(), String> {
    ...
    if completed {
        return Err("Reabra as atividades concluídas antes de excluir o roadmap".into());
    }
    if connection
        .execute("DELETE FROM study_roadmaps WHERE id=?1", [id])
        .map_err(err)?
        == 0
    {
        return Err("Roadmap não encontrado".into());
    }
    Ok(())
}
    ...
    let rows = statement
        .query_map([], |row| {
            Ok(ResearchItem {
                ...
                status: row.get(6)?,
                impact: row.get(7)?,
                knowledge_node_id: row.get(8)?,
                roadmap_id: row.get(9)?,
                roadmap_topic_id: row.get(10)?,
...
pub fn save_research(connection: &Connection, input: &ResearchItem) -> Result<(), String> {
    required(&input.id, "o ID da pesquisa")?;
    required(&input.title, "o título da pesquisa")?;
    if !["project", "study", "research"].contains(&input.kind.as_str()) {
        return Err("Tipo de pesquisa inválido".into());
    }
    if !["planned", "active", "paused", "completed"].contains(&input.status.as_str()) {
        return Err("Status de pesquisa inválido".into());
    }
    if input.roadmap_topic_id.is_some() && input.roadmap_id.is_none() {
        return Err("Um tópico de roadmap exige o roadmap relacionado".into());
    }
    if let (Some(topic_id), Some(roadmap_id)) = (&input.roadmap_topic_id, &input.roadmap_id) {
        ...
        if !association {
            return Err("O tópico selecionado não pertence ao roadmap informado".into());
        }
    }
...
pub fn delete_research(connection: &Connection, id: &str) -> Result<(), String> {
    if connection
        .execute("DELETE FROM research_items WHERE id=?1", [id])
        .map_err(err)?
        == 0
    {
        return Err("Pesquisa não encontrada".into());
    }
    Ok(())
}
...
pub fn summary(connection: &Connection) -> Result<StarkSummary, String> {
    ...
    Ok(StarkSummary {
        roadmap_count: count("SELECT COUNT(*) FROM study_roadmaps")?,
        active_roadmap_count: count("SELECT COUNT(*) FROM study_roadmaps WHERE status='active'")?,
        research_count: count("SELECT COUNT(*) FROM research_items")?,
        active_research_count: count("SELECT COUNT(*) FROM research_items WHERE status='active'")?,
        baseline_count: count("SELECT COUNT(*) FROM knowledge_baselines")?,
        event_count: count("SELECT COUNT(*) FROM knowledge_events")?,
    })
}
...
mod tests {
    ...
    fn roadmap() -> StudyRoadmapInput {
        let activity =
            |id: &str, title: &str, kind: &str, status: &str, order: i64| RoadmapActivity {
                id: id.into(),
                title: title.into(),
                description: "".into(),
        ...
        StudyRoadmapInput {
            ...
            name: "Controle e Automação".into(),
            description: "Teste".into(),
            status: "active".into(),
            stages: vec![RoadmapStage {
                id: "electronics-stage".into(),
                name: "Eletrônica".into(),
                description: "".into(),
                order: 1,
                topics: vec![RoadmapTopic {
                    ...
                    prerequisite_topic_ids: vec![],
                    activities: vec![
                        activity("read-mosfet", "Ler fundamentos", "READING", "completed", 1),
                        activity(
                            "simulate-mosfet",
                            "Simular circuito",
                            "SIMULATION",
    ...
    fn roadmap_completion_creates_evidence_and_updates_knowledge() {
        ...
        assert_eq!(
            (
                result[0].completed_activities,
                result[0].total_activities,
                result[0].progress
            ),
            (1, 2, 50)
        );
        ...
        assert!(delete_roadmap(&connection, "control-roadmap")
            .unwrap_err()
            .contains("conclu"));
        let event_count = list_events(&connection).unwrap().len();
        assert!(save_roadmap(&mut connection, &roadmap())
            .unwrap()
            .created_events
            .is_empty());
        assert_eq!(list_events(&connection).unwrap().len(), event_count);
        let mut reopened = roadmap();
        reopened.stages[0].topics[0].activities[0].status = "pending".into();
        ...
        assert!(recompleted
            .created_events
            .iter()
            .all(|event| event.evidence_cycle == 2));
    }
    ...
    fn direct_activity_transition_is_atomic_and_idempotent() {
        ...
        database::initialize(&mut connection).unwrap();
        let mut input = roadmap();
        input.stages[0].topics[0].activities[0].status = "pending".into();
        input.stages[0].topics[0].activities[0].completed_at = None;
        save_roadmap(&mut connection, &input).unwrap();
        let started = update_activity_status(
            &mut connection,
            &RoadmapActivityStatusInput {
                activity_id: "read-mosfet".into(),
                status: "in_progress".into(),
            },
        )
        .unwrap();
        assert!(started.learning.created_events.is_empty());
        let completed = update_activity_status(
            &mut connection,
            &RoadmapActivityStatusInput {
                activity_id: "read-mosfet".into(),
        ...
        let duplicate = update_activity_status(
            &mut connection,
            &RoadmapActivityStatusInput {
                activity_id: "read-mosfet".into(),
                status: "completed".into(),
        ...
        let reopened = update_activity_status(
            &mut connection,
            &RoadmapActivityStatusInput {
                activity_id: "read-mosfet".into(),
                status: "pending".into(),
        ...
        assert!(reopened
            ...
            .created_events
            .iter()
            .any(|event| event.event_type == "activity_reopened"));
    ...
    fn roadmap_prerequisites_are_structured_and_validated() {
        ...
        database::initialize(&mut connection).unwrap();
        let mut input = roadmap();
        let mut second = input.stages[0].topics[0].clone();
        second.id = "driver-topic".into();
        second.name = "Drivers".into();
        second.order = 2;
        second.activities.clear();
        second.prerequisite_topic_ids = vec!["mosfet-topic".into()];
        input.stages[0].topics.push(second);
        save_roadmap(&mut connection, &input).unwrap();
        let saved = list_roadmaps(&connection).unwrap();
        assert_eq!(
            saved[0].stages[0].topics[1].prerequisite_topic_ids,
            vec!["mosfet-topic"]
        );
        input.stages[0].topics[1].prerequisite_topic_ids = vec!["inexistente".into()];
        assert!(save_roadmap(&mut connection, &input)
            .unwrap_err()
            .contains("Pré-requisito"));
    }
    ...
    fn roadmap_rejects_unknown_knowledge_before_writing() {
        ...
        database::initialize(&mut connection).unwrap();
        let mut input = roadmap();
        input.stages[0].topics[0].knowledge_node_id = Some("unknown".into());
        assert!(save_roadmap(&mut connection, &input)
            .unwrap_err()
    ...
    fn research_relationships_persist_without_creating_events() {
        ...
        database::initialize(&mut connection).unwrap();
        save_roadmap(&mut connection, &roadmap()).unwrap();
        let input = ResearchItem {
            id: "pid-research".into(),
            title: "Controle PID".into(),
            domain: "Controle".into(),
            objective: "Investigar".into(),
            description: "".into(),
            kind: "research".into(),
            status: "completed".into(),
            impact: "".into(),
        ...
        let saved = list_research(&connection)
            .unwrap()
            .into_iter()
