...
pub fn evidence_profile(kind: &str) -> Option<EvidenceProfile> {
    Some(match kind {
        "READING" => EvidenceProfile {
            coverage: 1.0,
            depth: 0.1,
            integration: 0.0,
        },
        "LESSON" => EvidenceProfile {
            coverage: 0.9,
            depth: 0.2,
            integration: 0.0,
        },
        "QUIZ" => EvidenceProfile {
            coverage: 0.6,
            depth: 0.35,
            integration: 0.0,
        },
        "EXERCISE" => EvidenceProfile {
            coverage: 0.45,
            depth: 0.7,
            integration: 0.1,
        },
        "SIMULATION" => EvidenceProfile {
            coverage: 0.35,
            depth: 0.85,
            integration: 0.3,
        },
        ...
        "PROJECT" => EvidenceProfile {
            coverage: 0.4,
            depth: 1.0,
            integration: 1.0,
        },
        "DOCUMENTATION" => EvidenceProfile {
            coverage: 0.55,
            depth: 0.6,
            integration: 0.35,
        },
        "RESEARCH" => EvidenceProfile {
            coverage: 0.7,
            depth: 0.45,
            integration: 0.3,
        },
        "OTHER" => EvidenceProfile {
            coverage: 0.3,
            depth: 0.3,
            integration: 0.1,
        },
...
pub fn repetition_factor(index: usize) -> f64 {
    match index {
        ...
        3 => 0.40,
        4 => 0.30,
        value => (0.30 / ((value - 3) as f64).sqrt()).max(0.10),
    }
}
pub fn diversity_factor(distinct_types: usize) -> f64 {
    (1.0 + distinct_types.saturating_sub(1) as f64 * 0.04).min(1.24)
}
pub fn saturation_factor(current: f64) -> f64 {
    ((100.0 - current.clamp(0.0, 100.0)) / 100.0).max(0.02)
}
...
fn ancestors(connection: &Connection, node_id: &str) -> Result<Vec<(String, usize)>, String> {
    ...
    for depth in 1..HIERARCHY_FACTORS.len() {
        let parent = connection
            .query_row(
                "SELECT parent_id FROM knowledge_areas WHERE id=?1",
                [&current],
        ...
        if !seen.insert(parent.clone()) {
            return Err("Hierarquia de conhecimento contém ciclo".into());
        }
...
fn load_event(row: &rusqlite::Row<'_>) -> rusqlite::Result<KnowledgeEvent> {
    Ok(KnowledgeEvent {
        id: row.get(0)?,
        knowledge_node_id: row.get(1)?,
        source_type: row.get(2)?,
        source_id: row.get(3)?,
        event_type: row.get(4)?,
        coverage_delta: row.get(5)?,
        depth_delta: row.get(6)?,
        integration_delta: row.get(7)?,
        description: row.get(8)?,
        created_at: row.get(9)?,
        activity_type: row.get(10)?,
        roadmap_id: row.get(11)?,
        topic_id: row.get(12)?,
        evidence_cycle: row.get(13)?,
        formula_version: row.get(14)?,
        coverage_impact: row.get(15)?,
        depth_impact: row.get(16)?,
        integration_impact: row.get(17)?,
        metadata_json: row.get(18)?,
        reversal_of_event_id: row.get(19)?,
    })
}
...
pub fn complete_activity(connection: &Connection, activity_id: &str) -> Result<(), String> {
    ...
    let profile =
        evidence_profile(&kind).ok_or_else(|| "Tipo de evidência inválido".to_string())?;
    ...
    if direct.is_empty() {
        return Err(format!(
            "A atividade {title} não possui conhecimento relacionado"
        ));
    }
    ...
    let secondary_count = direct
        .iter()
        .filter(|(_, role)| role == "secondary")
        .count();
    let secondary_factor = if secondary_count <= 2 {
        0.5
    } else {
        1.0 / secondary_count as f64
    };
    ...
    for (node, role) in &direct {
        ...
        for (target, depth) in ancestors(connection, node)? {
            ...
            targets
                ...
                .and_modify(|entry| {
                    if factor > entry.0 {
                        *entry = (factor, direct_target);
                    }
                })
                .or_insert((factor, direct_target));
        }
    }
    for (node, (distribution, is_direct)) in targets {
        ...
        let rep = repetition_factor(repetition as usize);
        let diversity = diversity_factor((distinct + 1) as usize);
        ...
        let coverage = profile.coverage
            ...
            * diversity
            * distribution
            * saturation_factor(current_coverage);
        let depth = profile.depth
            * BASE_IMPACT
            * rep
            * diversity
            * distribution
            * saturation_factor(current_depth);
        let integration = if direct.len() > 1 && is_direct {
            profile.integration * BASE_IMPACT * rep * diversity * distribution
                / direct
                    .iter()
                    .map(|(_, role)| {
                        if role == "primary" {
...
pub fn reopen_activity(connection: &Connection, activity_id: &str) -> Result<(), String> {
    ...
    for (original, node, kind, roadmap, topic, cycle, coverage, depth, integration, title) in
        originals
    {
        let metadata =
            serde_json::json!({"reversalOf":original,"formulaVersion":FORMULA_VERSION}).to_string();
        insert_event(
            connection,
            &node,
            activity_id,
...
pub fn manual_adjustment(
    ...
    let previous: HashSet<String> = list_events(connection)?
        .into_iter()
        .map(|event| event.id)
        .collect();
    let (current_coverage, current_depth) = current_metric(connection, knowledge_id)?;
    let coverage_impact = coverage as f64 - current_coverage;
    let depth_impact = depth as f64 - current_depth;
    ...
    result.created_events = list_events(connection)?
        .into_iter()
        .filter(|event| !previous.contains(&event.id))
        .collect();
    Ok(result)
}
...
pub fn recalculate(connection: &Connection, reason: &str) -> Result<LearningMutation, String> {
    ...
    for (node, base_c, base_d, old_c, old_d) in nodes {
        ...
        let new_c = (base_c + delta_c).clamp(0.0, 100.0).round() as i64;
        let new_d = (base_d + delta_d).clamp(0.0, 100.0).round() as i64;
    ...
    connection
        .execute(
            "UPDATE app_metrics SET value=?1 WHERE key='integration'",
            [integration],
        )
        .map_err(|e| e.to_string())?;
...
fn derive_topic_mastery(connection: &Connection) -> Result<(), String> {
    let mut statement = connection
        .prepare("SELECT id,knowledge_node_id FROM roadmap_topics")
        .map_err(|e| e.to_string())?;
    ...
    for (topic, node) in topics {
        let mut kinds_stmt = connection
            .prepare(&format!(
                "SELECT DISTINCT activity_type FROM knowledge_events WHERE topic_id=?1 AND {}",
                active_evidence_filter()
            ))
            .map_err(|e| e.to_string())?;
        let kinds: HashSet<String> = kinds_stmt
        ...
        if kinds
            .iter()
            .any(|k| ["READING", "LESSON", "RESEARCH"].contains(&k.as_str()))
        {
            state = "EXPOSED";
        }
        if kinds
            .iter()
            .any(|k| ["QUIZ", "EXERCISE"].contains(&k.as_str()))
        {
            state = "UNDERSTOOD";
        }
        if kinds
            .iter()
            .any(|k| ["SIMULATION", "EXPERIMENT"].contains(&k.as_str()))
        {
            state = "PRACTICED";
        }
        ...
        if let Some(node) = node {
            let (c, d) = current_metric(connection, &node)?;
            if c >= 85.0 && d >= 80.0 && kinds.len() >= 5 && kinds.contains("PROJECT") {
                state = "MASTERED";
            }
        }
        connection
            .execute(
                "UPDATE roadmap_topics SET topic_state=?1 WHERE id=?2",
                params![state, topic],
            )
            .map_err(|e| e.to_string())?;
    }
...
mod tests {
    ...
    fn profiles_are_distinct() {
        assert!(
            evidence_profile("READING").unwrap().coverage
                > evidence_profile("READING").unwrap().depth
        );
        assert!(
            evidence_profile("EXPERIMENT").unwrap().depth
                > evidence_profile("EXPERIMENT").unwrap().coverage
        );
        assert_eq!(evidence_profile("PROJECT").unwrap().integration, 1.0);
    }
    #[test]
    fn diminishing_diversity_and_saturation_are_bounded() {
        assert!(repetition_factor(0) > repetition_factor(9));
        assert!(diversity_factor(10) <= 1.24);
        assert!(saturation_factor(90.0) < saturation_factor(20.0));
    }
}
