...
fn percentage(numerator: usize, denominator: usize) -> f64 {
    if denominator == 0 {
        ...
        rounded(numerator as f64 / denominator as f64 * 100.0)
    }
}
...
fn pearson(left: &[f64], right: &[f64]) -> Option<f64> {
    ...
    let right_variance = right
        ...
        .sum::<f64>();
    if left_variance <= EPSILON || right_variance <= EPSILON {
        None
    } else {
        Some(rounded(
...
fn equity_returns(snapshots: &[Snapshot]) -> Vec<f64> {
    snapshots
        .windows(2)
        .map(|pair| {
            if pair[0].equity.abs() <= EPSILON {
                0.0
            } else {
                rounded(pair[1].equity / pair[0].equity - 1.0)
            }
        })
        .collect()
}
...
fn build_episodes(agent_id: &str, snapshots: &[Snapshot]) -> Vec<MarketPositionEpisode> {
    ...
    for snapshot in snapshots {
        ...
        } else if let Some((episode_index, opened_index, opened_at, duration, max_exposure)) =
            active.take()
        {
            episodes.push(MarketPositionEpisode {
                agent_id: agent_id.into(),
                episode_index,
                opened_candle_index: opened_index,
                opened_at,
                closed_candle_index: Some(snapshot.candle_index),
                closed_at: Some(snapshot.timestamp.clone()),
                duration_candles: duration,
                max_exposure_pct: rounded(max_exposure),
                status: "closed".into(),
            });
        }
    }
...
fn load_agents(connection: &Connection, experiment_id: &str) -> Result<Vec<String>, String> {
    ...
    let rows = statement
        .query_map([experiment_id], |row| row.get(0))
        .map_err(|error| error.to_string())?
...
fn load_snapshots(
    ...
    let rows = statement
        .query_map(params![experiment_id, agent_id], |row| {
            Ok(Snapshot {
                candle_index: row.get(0)?,
                timestamp: row.get(1)?,
                quantity: row.get(2)?,
                equity: row.get(3)?,
                exposure_pct: row.get(4)?,
            })
        })
        .map_err(|error| error.to_string())?
        .collect::<Result<Vec<_>, _>>()
...
fn load_decisions(
    ...
    let rows = statement
        .query_map(params![experiment_id, agent_id], |row| {
            Ok(Decision {
                action: row.get(0)?,
                risk_result: row.get(1)?,
                risk_reason: row.get(2)?,
            })
        })
        .map_err(|error| error.to_string())?
        .collect::<Result<Vec<_>, _>>()
...
pub fn calculate(
    connection: &Connection,
    experiment_id: &str,
) -> Result<MarketObservatory, String> {
    let agents = load_agents(connection, experiment_id)?;
    let returns_by_agent: HashMap<String, f64> = {
        let mut statement = connection
            .prepare(
                "SELECT agent_id,total_return_pct FROM market_agent_metrics WHERE experiment_id=?1",
            )
            .map_err(|error| error.to_string())?;
        let rows = statement
            .query_map([experiment_id], |row| Ok((row.get(0)?, row.get(1)?)))
            .map_err(|error| error.to_string())?
    ...
    for agent_id in &agents {
        ...
        let mut durations = agent_episodes
            .iter()
            .map(|episode| episode.duration_candles)
            .collect::<Vec<_>>();
        let average_holding = if durations.is_empty() {
            0.0
        } else {
            rounded(durations.iter().sum::<usize>() as f64 / durations.len() as f64)
        };
        ...
        let rejected = decisions
            .iter()
            .filter(|decision| decision.risk_result == "REJECTED")
            .count();
        let modified = decisions
            .iter()
            .filter(|decision| decision.risk_result == "MODIFIED")
            .count();
        let drawdown_triggers = decisions
            .iter()
            .filter(|decision| decision.risk_reason.to_lowercase().contains("drawdown"))
            .count();
        let daily_loss_triggers = decisions
            .iter()
            .filter(|decision| decision.risk_reason.to_lowercase().contains("perda diária"))
            .count();
        let exposed = snapshots
            .iter()
            .filter(|snapshot| snapshot.quantity > EPSILON)
            .collect::<Vec<_>>();
        let average_exposure = if snapshots.is_empty() {
            0.0
        } else {
            rounded(
                snapshots
                    .iter()
                    .map(|snapshot| snapshot.exposure_pct)
                    .sum::<f64>()
                    / snapshots.len() as f64,
            )
        };
        let average_position = if exposed.is_empty() {
            0.0
        } else {
            rounded(
                exposed
        ...
        let max_exposure = snapshots
            .iter()
            .map(|snapshot| snapshot.exposure_pct)
            .fold(0.0, f64::max);
        let average_equity = if snapshots.is_empty() {
            0.0
        } else {
            snapshots
                .iter()
        ...
        let (execution_count, entry_count, exit_count, gross_traded) =
            execution_summary(connection, experiment_id, agent_id)?;
        let turnover = if average_equity.abs() <= EPSILON {
            0.0
        } else {
            rounded(gross_traded / average_equity * 100.0)
        };
        behavior.push(MarketBehaviorMetric {
            ...
            max_exposure_pct: rounded(max_exposure),
            average_position_size_pct: average_position,
            max_position_size_pct: rounded(max_exposure),
            average_holding_candles: average_holding,
            median_holding_candles: median_holding,
            max_holding_candles: max_holding,
            turnover_pct: turnover,
            time_in_market_pct: percentage(exposed.len(), snapshots.len()),
            time_in_cash_pct: percentage(
                snapshots.len().saturating_sub(exposed.len()),
                snapshots.len(),
            ),
            entry_count,
            exit_count,
            risk_rejection_count: rejected,
            risk_modification_count: modified,
            risk_rejection_rate_pct: percentage(rejected, decisions.len()),
            risk_modification_rate_pct: percentage(modified, decisions.len()),
            drawdown_trigger_count: drawdown_triggers,
            daily_loss_trigger_count: daily_loss_triggers,
    ...
    for (left_index, left_id) in agents.iter().enumerate() {
        for right_id in agents.iter().skip(left_index) {
            ...
            let equal_decisions = left_actions
                .iter()
                .zip(right_actions)
                .filter(|(left, right)| left.action == right.action)
                .count();
            let decision_similarity = if diagonal {
                1.0
            } else if decision_count == 0 {
                0.0
            } else {
                rounded(equal_decisions as f64 / decision_count as f64)
            };
            let equity_return_correlation = if diagonal {
                Some(1.0)
            } else {
                pearson(
                    &equity_returns(&snapshots_by_agent[left_id]),
                    &equity_returns(&snapshots_by_agent[right_id]),
                )
            };
            let high_similarity = !diagonal
                && (decision_similarity >= SIMILARITY_THRESHOLD
                    || equity_return_correlation
                        .is_some_and(|value| value >= SIMILARITY_THRESHOLD));
            correlations.push(MarketAgentCorrelation {
                agent_a_id: left_id.clone(),
                agent_b_id: right_id.clone(),
                equity_return_correlation,
                decision_similarity,
                high_similarity,
    ...
    let benchmarks = agents
        .iter()
        .map(|agent_id| {
            let agent_return = returns_by_agent.get(agent_id).copied().unwrap_or(0.0);
            MarketBenchmarkComparison {
                agent_id: agent_id.clone(),
                cash_return_pct: cash_return,
                buy_hold_return_pct: buy_hold_return,
                excess_vs_cash_pct: rounded(agent_return - cash_return.unwrap_or(0.0)),
                excess_vs_buy_hold_pct: buy_hold_return.map(|value| rounded(agent_return - value)),
            }
        })
        .collect();
    Ok(MarketObservatory {
        behavior,
        episodes,
        correlations,
        benchmarks,
...
pub fn load(connection: &Connection, experiment_id: &str) -> Result<MarketObservatory, String> {
    ...
    let behavior = {
        ...
        let rows = statement
            .query_map([experiment_id], |row| {
                Ok(MarketBehaviorMetric {
                    agent_id: row.get(0)?,
                    buy_count: row.get(1)?,
                    sell_count: row.get(2)?,
                    hold_count: row.get(3)?,
                    hold_rate_pct: row.get(4)?,
                    trade_frequency_pct: row.get(5)?,
                    average_exposure_pct: row.get(6)?,
                    max_exposure_pct: row.get(7)?,
                    average_position_size_pct: row.get(8)?,
                    max_position_size_pct: row.get(9)?,
                    average_holding_candles: row.get(10)?,
                    median_holding_candles: row.get(11)?,
                    max_holding_candles: row.get(12)?,
                    turnover_pct: row.get(13)?,
                    time_in_market_pct: row.get(14)?,
                    time_in_cash_pct: row.get(15)?,
                    entry_count: row.get(16)?,
                    exit_count: row.get(17)?,
                    risk_rejection_count: row.get(18)?,
                    risk_modification_count: row.get(19)?,
                    risk_rejection_rate_pct: row.get(20)?,
                    risk_modification_rate_pct: row.get(21)?,
                    drawdown_trigger_count: row.get(22)?,
                    daily_loss_trigger_count: row.get(23)?,
                    formula_version: row.get(24)?,
                })
            })
            .map_err(|error| error.to_string())?
            .collect::<Result<Vec<_>, _>>()
    ...
    let episodes = {
        ...
        let rows = statement
            .query_map([experiment_id], |row| {
                Ok(MarketPositionEpisode {
                    agent_id: row.get(0)?,
                    episode_index: row.get(1)?,
                    opened_candle_index: row.get(2)?,
                    opened_at: row.get(3)?,
                    closed_candle_index: row.get(4)?,
                    closed_at: row.get(5)?,
                    duration_candles: row.get(6)?,
                    max_exposure_pct: row.get(7)?,
                    status: row.get(8)?,
                })
            })
    ...
    let correlations = {
        ...
        let rows = statement
            .query_map([experiment_id], |row| {
                Ok(MarketAgentCorrelation {
                    agent_a_id: row.get(0)?,
                    agent_b_id: row.get(1)?,
                    equity_return_correlation: row.get(2)?,
                    decision_similarity: row.get(3)?,
                    high_similarity: row.get(4)?,
                    similarity_threshold: row.get(5)?,
                })
            })
            .map_err(|error| error.to_string())?
            .collect::<Result<Vec<_>, _>>()
    ...
    let benchmarks = {
        ...
        let rows = statement
            .query_map([experiment_id], |row| {
                Ok(MarketBenchmarkComparison {
                    agent_id: row.get(0)?,
                    cash_return_pct: row.get(1)?,
                    buy_hold_return_pct: row.get(2)?,
                    excess_vs_cash_pct: row.get(3)?,
                    excess_vs_buy_hold_pct: row.get(4)?,
                })
            })
            .map_err(|error| error.to_string())?
            .collect::<Result<Vec<_>, _>>()
...
pub fn persist(connection: &Connection, experiment_id: &str) -> Result<MarketObservatory, String> {
    ...
    connection
        .execute(
            "DELETE FROM market_behavior_metrics WHERE experiment_id=?1",
            [experiment_id],
        )
        .map_err(|error| error.to_string())?;
    connection
        .execute(
            "DELETE FROM market_position_episodes WHERE experiment_id=?1",
            [experiment_id],
        )
        .map_err(|error| error.to_string())?;
    connection
        .execute(
            "DELETE FROM market_agent_correlations WHERE experiment_id=?1",
            [experiment_id],
        )
        .map_err(|error| error.to_string())?;
    connection
        .execute(
            "DELETE FROM market_comparison_snapshots WHERE experiment_id=?1",
            [experiment_id],
        )
        .map_err(|error| error.to_string())?;
    ...
    for correlation in &observatory.correlations {
        connection
            .execute(
                "INSERT INTO market_agent_correlations VALUES(?1,?2,?3,?4,?5,?6,?7,?8)",
                params![
                    experiment_id,
                    correlation.agent_a_id,
                    correlation.agent_b_id,
                    correlation.equity_return_correlation,
                    correlation.decision_similarity,
                    correlation.high_similarity,
                    correlation.similarity_threshold,
                    FORMULA_VERSION
                ],
            )
            .map_err(|error| error.to_string())?;
