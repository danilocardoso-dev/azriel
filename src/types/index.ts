...
export interface MarketAiStatus { configured: boolean; available: boolean; provider: string; model: string; promptVersion: string; decisionInterval: number; timeoutMs: number; maxRetries: number; error: string | null }
export interface UpdateMarketAiConfigInput { decisionInterval: number; timeoutMs: number; maxRetries: number }
export interface MarketAiRuntimeMetric { agentId: string; callCount: number; successfulCallCount: number; invalidResponseCount: number; timeoutCount: number; retryCount: number; fallbackCount: number; averageLatencyMs: number; maxLatencyMs: number; totalLatencyMs: number; inputTokens: number | null; outputTokens: number | null }
export interface MarketAiDecisionLog { decisionId: number; timestamp: string; action: "BUY" | "SELL" | "HOLD"; desiredPositionPct: number | null; confidence: number | null; reason: string; riskResult: "APPROVED" | "MODIFIED" | "REJECTED"; riskReason: string; approvedPositionPct: number | null; executionPrice: number | null; callStatus: "VALID" | "INVALID" | "TIMEOUT" | "OFFLINE" | "NO_LLM_CALL"; provider: string; model: string; promptVersion: string; latencyMs: number; attempts: number; fallbackUsed: boolean }
