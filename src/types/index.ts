...
export interface MarketAiRuntimeMetric { agentId: string; promptVersion: string; callCount: number; successfulCallCount: number; invalidResponseCount: number; timeoutCount: number; retryCount: number; fallbackCount: number; averageLatencyMs: number; maxLatencyMs: number; totalLatencyMs: number; inputTokens: number | null; outputTokens: number | null; buyCount: number; sellCount: number; llmHoldCount: number; noLlmCallCount: number; averageConfidence: number | null; averageBuyConfidence: number | null; averageSellConfidence: number | null; averageHoldConfidence: number | null; minConfidence: number | null; maxConfidence: number | null; medianConfidence: number | null; confidenceDistribution: number[] }
export interface MarketAiDecisionLog { decisionId: number; timestamp: string; action: "BUY" | "SELL" | "HOLD"; lifecycleAction: "ENTER_LONG" | "HOLD_POSITION" | "INCREASE_LONG" | "REDUCE_LONG" | "EXIT_LONG" | "STAY_FLAT" | null; desiredPositionPct: number | null; confidence: number | null; reason: string; reasonCode: string | null; validationCode: string | null; riskResult: "APPROVED" | "MODIFIED" | "REJECTED"; riskReason: string; approvedPositionPct: number | null; executionPrice: number | null; callStatus: "VALID" | "INVALID" | "TIMEOUT" | "OFFLINE" | "NO_LLM_CALL"; provider: string; model: string; promptVersion: string; latencyMs: number; attempts: number; fallbackUsed: boolean; inputSnapshot: Record<string, unknown> | null; forwardReturn1: number | null; forwardReturn5: number | null; forwardReturn10: number | null; signalDisagreement: boolean }
export interface MarketAiExperimentComparison { experimentId: string; experimentName: string; datasetId: string; datasetName: string; riskProfileId: string; initialCapital: number; randomSeed: number; feePct: number; slippagePct: number; decisionInterval: number; agentId: string; promptVersion: string; callCount: number; successfulCallCount: number; invalidResponseCount: number; timeoutCount: number; buyCount: number; sellCount: number; llmHoldCount: number; tradeCount: number; holdRatePct: number; averageConfidence: number | null; totalReturnPct: number; maxDrawdownPct: number; averageExposurePct: number; averageLatencyMs: number }
...
export interface MarketExperimentResult { experiment: MarketExperimentSummary; metrics: MarketAgentMetric[]; equity: MarketEquityPoint[]; decisions: MarketDecisionLog[]; observatory: MarketObservatory; positionLifecycle: MarketPositionLifecycleReport }
