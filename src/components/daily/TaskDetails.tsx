...
export function TaskDetails({ task, onClear }: { task: Task; onClear: () => void }) {
  ...
  const [form, setForm] = useState<TaskInput>({ id: task.id, title: task.title, description: task.description, status: task.status, priority: task.priority, dueDate: task.dueDate, projectId: task.projectId, knowledgeAreaId: task.knowledgeAreaId });
  ...
  async function submit(event: FormEvent) {
    ...
    try { await saveTask(form); setMessage("Tarefa atualizada."); }
  ...
  return <form className="daily-details" onSubmit={submit}>
    <header><span>TASK // {task.id.slice(0, 8).toUpperCase()}</span><strong>DETALHES</strong></header>
    <label>Título<input value={form.title} onChange={(event) => change("title", event.target.value)} /></label>
    <label>Descrição<textarea rows={5} value={form.description} onChange={(event) => change("description", event.target.value)} /></label>
    <div className="daily-details__split">
    ...
    <label>Prazo<input type="date" value={form.dueDate ?? ""} onChange={(event) => change("dueDate", event.target.value || null)} /></label>
    ...
    <div className="daily-timestamps"><span>CRIADA<strong>{formatTimestamp(task.createdAt)}</strong></span><span>ATUALIZADA<strong>{formatTimestamp(task.updatedAt)}</strong></span>{task.completedAt && <span>CONCLUÍDA<strong>{formatTimestamp(task.completedAt)}</strong></span>}</div>
    <div className="daily-details__actions"><button disabled={busy}>SALVAR</button>{task.status !== "completed" && <button type="button" onClick={() => void complete()} disabled={busy}>CONCLUIR</button>}<button type="button" className="danger" onClick={() => setConfirmDelete(true)} disabled={busy}>EXCLUIR</button></div>
