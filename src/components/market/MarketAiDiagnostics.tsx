...
function RiskDecisionTrace({ decision }: { decision: MarketAiDecisionLog }) {
  const trace = decision.riskTrace;
  return <section className="market-ai-pipeline market-risk-trace">
    <div className="panel-heading"><strong>RISK DECISION TRACE</strong><span>{trace.policyVersion}</span></div>
    <div className="market-ai-runtime__metrics">
      <div><span>CLASSIFICATION</span><strong>{trace.classification}</strong></div>
      <div><span>CURRENT EXPOSURE</span><strong>{trace.currentExposurePct.toFixed(4)}%</strong></div>
      <div><span>TARGET EXPOSURE</span><strong>{trace.targetExposurePct.toFixed(4)}%</strong></div>
      <div><span>EXPOSURE CHANGE</span><strong>{trace.exposureDeltaPct >= 0 ? "+" : ""}{trace.exposureDeltaPct.toFixed(4)} p.p.</strong></div>
    </div>
    {trace.rules.length === 0 ? <p>Registro anterior à política estruturada; sequência de regras indisponível.</p> : <div className="market-ai-pipeline__stages">{trace.rules.map((rule, index) => <div key={`${rule.rule}-${index}`} className={rule.status === "REJECTED" ? "fail" : "pass"}><span>{rule.rule.replaceAll("_", " ")}</span><strong>{rule.status}</strong><small>{rule.reason ?? "OK"}</small></div>)}</div>}
    <div className="market-ai-pipeline__final"><div><span>FINAL RESULT</span><strong>{trace.result}</strong></div><div><span>APPROVED TARGET</span><strong>{trace.approvedPositionPct?.toFixed(4) ?? "N/A"}%</strong></div><div><span>EXACT REASON</span><strong>{trace.reason}</strong></div></div>
  </section>;
}
...
export function AiRuntimePanel({ status, runtime, decisions }: { status: MarketAiStatus | null; runtime: MarketAiRuntimeMetric | null; decisions: MarketAiDecisionLog[] }) {
  ...
  return <section className="market-ai-runtime">
    ...
    {selected && <RiskDecisionTrace decision={selected} />}
