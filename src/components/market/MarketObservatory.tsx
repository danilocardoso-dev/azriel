...
function PositionLifecycle({ lifecycles, events, metrics, aiDecisions }: { lifecycles: MarketTradeLifecycle[]; events: MarketPositionEvent[]; metrics?: MarketPositionMetric; aiDecisions: MarketAiDecisionLog[] }) {
  ...
  return <div className="market-position-lifecycle">
    ...
    <div className="market-position-table"><table><thead><tr><th>TRADE</th><th>ENTRY</th><th>EXIT</th><th>DURATION</th><th>ENTRY EXP.</th><th>MAX EXP.</th><th>EXIT EXP.</th><th>PNL</th><th>MFE</th><th>MAE</th><th>GIVEBACK (P.P.)</th><th>EXIT EFF.</th><th>ENTRY REASON</th><th>EXIT REASON</th></tr></thead><tbody>{lifecycles.map((item) => <tr key={item.id} className={item.id === selected?.id ? "selected" : ""} onClick={() => setSelectedId(item.id)}><td>#{String(item.lifecycleIndex).padStart(2, "0")}</td><td>{item.openedAt}</td><td>{item.closedAt ?? "OPEN"}</td><td>{item.holdingCandles}</td><td>{number(item.initialExposurePct)}%</td><td>{number(item.maxExposurePct)}%</td><td>{number(exitExposure(item))}%</td><td className={item.realizedPnlPct >= 0 ? "positive" : "negative"}>{number(item.realizedPnlPct)}%</td><td>{number(item.mfePct)}%</td><td>{number(item.maePct)}%</td><td>{number(item.profitGivebackPct)} p.p.</td><td>{item.exitEfficiencyPct == null ? "N/A" : `${number(item.exitEfficiencyPct)}%`}</td><td>{item.entryReasonCode ?? "N/A"}</td><td>{item.exitReasonCode ?? (item.status === "OPEN" ? "OPEN" : "N/A")}</td></tr>)}</tbody></table></div>
