...
export function MarketValidation({ datasets, agents, profiles, datasetId, riskProfileId, agentIds, initialCapital, randomSeed, feePct, slippagePct, onError }: Props) {
  ...
  const [aiRuntime, setAiRuntime] = useState<MarketAiRuntimeMetric[]>([]);
  ...
  const run = async () => {
    ...
    setBusy(true);
    try { const next = await marketLabRepository.runValidation(input); const runtime=await marketLabRepository.listValidationAiRuntime(next.validation.id); setAiRuntime(runtime); setResult(next); setSelectedAgent(next.reports[0]?.agentId ?? ""); setHistory(await marketLabRepository.listValidations()); }
    catch (cause) { onError(String(cause)); }
    finally { setBusy(false); }
  };
  const open = async (id: string) => { setBusy(true); try { const [next,runtime] = await Promise.all([marketLabRepository.getValidation(id),marketLabRepository.listValidationAiRuntime(id)]); setAiRuntime(runtime); setResult(next); setSelectedAgent(next.reports[0]?.agentId ?? ""); } catch (cause) { onError(String(cause)); } finally { setBusy(false); } };
  ...
  return <div className="market-validation">
    ...
    {result && <>
      ...
      {aiRuntime.map((runtime)=><section key={runtime.agentId} className="market-ai-runtime__metrics market-ai-runtime__metrics--validation"><div><span>AI AGENT</span><strong>{runtime.agentId}</strong></div><div><span>AI CALLS</span><strong>{runtime.callCount}</strong></div><div><span>PROMPT</span><strong>{runtime.promptVersion}</strong></div><div><span>VALID</span><strong>{runtime.successfulCallCount}</strong></div><div><span>INVALID</span><strong>{runtime.invalidResponseCount}</strong></div><div><span>TIMEOUT</span><strong>{runtime.timeoutCount}</strong></div><div><span>FALLBACKS</span><strong>{runtime.fallbackCount}</strong></div><div><span>BUY</span><strong>{runtime.buyCount}</strong></div><div><span>SELL</span><strong>{runtime.sellCount}</strong></div><div><span>LLM HOLD</span><strong>{runtime.llmHoldCount}</strong></div><div><span>NO LLM CALL</span><strong>{runtime.noLlmCallCount}</strong></div><div><span>AVG LATENCY</span><strong>{runtime.averageLatencyMs.toFixed(0)} ms</strong></div></section>)}
