...
export function MarketValidation({ datasets, agents, profiles, datasetId, riskProfileId, agentIds, initialCapital, randomSeed, feePct, slippagePct, onError }: Props) {
  ...
  return <div className="market-validation">
    ...
    {result && <>
      ...
      {aiRuntime&&<section className="market-ai-runtime__metrics market-ai-runtime__metrics--validation"><div><span>AI CALLS</span><strong>{aiRuntime.callCount}</strong></div><div><span>PROMPT</span><strong>{aiRuntime.promptVersion}</strong></div><div><span>VALID</span><strong>{aiRuntime.successfulCallCount}</strong></div><div><span>INVALID</span><strong>{aiRuntime.invalidResponseCount}</strong></div><div><span>TIMEOUT</span><strong>{aiRuntime.timeoutCount}</strong></div><div><span>FALLBACKS</span><strong>{aiRuntime.fallbackCount}</strong></div><div><span>BUY</span><strong>{aiRuntime.buyCount}</strong></div><div><span>SELL</span><strong>{aiRuntime.sellCount}</strong></div><div><span>LLM HOLD</span><strong>{aiRuntime.llmHoldCount}</strong></div><div><span>NO LLM CALL</span><strong>{aiRuntime.noLlmCallCount}</strong></div><div><span>AVG CONFIDENCE</span><strong>{aiRuntime.averageConfidence?.toFixed(3)??"N/A"}</strong></div><div><span>AVG LATENCY</span><strong>{aiRuntime.averageLatencyMs.toFixed(0)} ms</strong></div></section>}
