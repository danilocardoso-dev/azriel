...
export function RoadmapEditor({ roadmap, knowledge, projects, research, onCancel, onSave }: Props) {
  ...
  const allTopics = form.stages.flatMap((stage) => stage.topics);
  const removeTopic = (topicId: string) => setForm((current) => ({ ...current, stages: current.stages.map((stage) => ({ ...stage, topics: stage.topics.filter((topic) => topic.id !== topicId).map((topic, index) => ({ ...topic, order: index + 1, prerequisiteTopicIds: (topic.prerequisiteTopicIds ?? []).filter((id) => id !== topicId) })) })) }));
  const removeStage = (stageIndex: number) => setForm((current) => { const removedIds = new Set(current.stages[stageIndex]?.topics.map((topic) => topic.id) ?? []); return { ...current, stages: current.stages.filter((_, index) => index !== stageIndex).map((stage, index) => ({ ...stage, order: index + 1, topics: stage.topics.map((topic) => ({ ...topic, prerequisiteTopicIds: (topic.prerequisiteTopicIds ?? []).filter((id) => !removedIds.has(id)) })) })) }; });
  ...
  return <RecordEditorDialog eyebrow="MAPA STARK // LEARNING ENGINE" title={roadmap ? `Editar ${roadmap.name}` : "Novo roadmap"} busy={busy} error={error} onCancel={onCancel} onSubmit={submit}>
    ...
    <div className="record-editor__wide roadmap-builder"><header><strong>ETAPAS, TÓPICOS E EVIDÊNCIAS</strong><button type="button" onClick={() => setForm((current) => ({ ...current, stages: [...current.stages, { id: uid(), name: "Nova etapa", description: "", order: current.stages.length + 1, topics: [] }] }))}>＋ ETAPA</button></header>
      {form.stages.map((stage, si) => <section className="roadmap-stage-editor" key={stage.id}>
        <div className="roadmap-builder__line"><b>ETAPA {si + 1}</b><input value={stage.name} onChange={(event) => changeStage(si, (item) => ({ ...item, name: event.target.value }))} /><button type="button" className="danger-link" onClick={() => removeStage(si)}>×</button></div>
        {stage.topics.map((topic, ti) => <article className="roadmap-topic-editor" key={topic.id}>
          <div className="roadmap-builder__line"><span>TÓPICO {ti + 1}</span><input value={topic.name} onChange={(event) => changeStage(si, (item) => ({ ...item, topics: item.topics.map((entry, index) => index === ti ? { ...entry, name: event.target.value } : entry) }))} /><button type="button" onClick={() => removeTopic(topic.id)}>×</button></div>
          <div className="roadmap-builder__fields"><label>Conhecimento padrão<select value={topic.knowledgeNodeId ?? ""} onChange={(event) => changeStage(si, (item) => ({ ...item, topics: item.topics.map((entry, index) => index === ti ? { ...entry, knowledgeNodeId: event.target.value || null } : entry) }))}><option value="">Selecione</option>{knowledge.map((area) => <option key={area.id} value={area.id}>{area.name}</option>)}</select></label><label>Domínio derivado<input value={topic.state} readOnly title="Calculado pelas evidências" /></label><label className="roadmap-prerequisites">Pré-requisitos<select multiple value={topic.prerequisiteTopicIds ?? []} onChange={(event) => changeStage(si, (item) => ({ ...item, topics: item.topics.map((entry, index) => index === ti ? { ...entry, prerequisiteTopicIds: [...event.target.selectedOptions].map((option) => option.value) } : entry) }))}>{allTopics.filter((item) => item.id !== topic.id).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><small>Use Ctrl para selecionar mais de um tópico.</small></label></div>
          {topic.activities.map((entry, ai) => <div className="roadmap-activity-editor learning-activity" key={entry.id}>
            ...
            <button type="button" onClick={() => changeStage(si, (item) => ({ ...item, topics: item.topics.map((current, index) => index === ti ? { ...current, activities: current.activities.filter((_, position) => position !== ai).map((activityItem, position) => ({ ...activityItem, order: position + 1 })) } : current) }))}>×</button>
        ...
        <button type="button" onClick={() => changeStage(si, (item) => ({ ...item, topics: [...item.topics, { id: uid(), name: "Novo tópico", description: "", knowledgeNodeId: null, state: "NOT_STARTED", order: item.topics.length + 1, prerequisiteTopicIds: [], activities: [] }] }))}>＋ TÓPICO</button>
      </section>)}
    </div>
  </RecordEditorDialog>;
}
