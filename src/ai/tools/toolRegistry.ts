...
function createTools(dependencies: ToolDependencies): AzrielTool[] {
  ...
  return [
    ...
    { name: "get_learning_progress", domain: "Learning Engine", description: "Progresso atual derivado de baseline e evidências", readonly: true, execute: async (input) => { const area = await findKnowledge(dependencies, input); const [areas, baselines, events] = await Promise.all([dependencies.knowledge.list(), stark.baselines(), stark.events()]); const selected = area ? [area] : areas; return selected.map((item) => ({ knowledge: item, baseline: baselines.find((entry) => entry.knowledgeAreaId === item.id) ?? null, evidenceCount: events.filter((event) => event.knowledgeNodeId === item.id).length })); } },
    ...
    { name: "get_knowledge_evidence", domain: "Learning Engine", description: "Evidências reais de um conhecimento", readonly: true, execute: async (input) => { const area=await findKnowledge(dependencies,input); if(!area)return []; return (await stark.events()).filter((event)=>event.knowledgeNodeId===area.id); } },
    { name: "get_recent_knowledge_events", domain: "Learning Engine", description: "Eventos recentes do ledger de aprendizado", readonly: true, execute: async () => (await stark.events()).slice(0,25) },
    { name: "explain_knowledge_level", domain: "Learning Engine", description: "Explica o nível com baseline, impactos e fontes reais", readonly: true, execute: async (input) => { const area=await findKnowledge(dependencies,input); if(!area)return null; const [baselines,events]=await Promise.all([stark.baselines(),stark.events()]); const evidence=events.filter((event)=>event.knowledgeNodeId===area.id); return {knowledge:area,baseline:baselines.find((item)=>item.knowledgeAreaId===area.id)??null,evidence,impact:evidence.reduce((sum,event)=>({coverage:sum.coverage+event.coverageImpact,depth:sum.depth+event.depthImpact,integration:sum.integration+event.integrationImpact}),{coverage:0,depth:0,integration:0})}; } },
    { name: "get_roadmap_learning_status", domain: "Learning Engine", description: "Contribuição de aprendizado por roadmap", readonly: true, execute: async (input) => { const [roadmaps,events]=await Promise.all([stark.roadmaps(),stark.events()]); const needle=normalize(input.term??input.query); return roadmaps.filter((item)=>!needle||normalize(item.name).includes(needle)).map((item)=>({roadmap:item.name,progress:item.progress,topics:item.stages.flatMap((stage)=>stage.topics).map((topic)=>({name:topic.name,state:topic.state})),evidence:events.filter((event)=>event.roadmapId===item.id)})); } },
    ...
    { name: "get_azriel_status", domain: "Azriel", description: "Estado consolidado do Azriel", readonly: true, execute: async () => { const [database, daily, system, workspaces, routines] = await Promise.all([dependencies.databaseInfo(), dependencies.tasks.counters(), dependencies.system.snapshot(), dependencies.system.listWorkspaces(), dependencies.automation.listRoutines()]); return { version: "0.8.3", database, daily, system: { cpuUsagePercent: system.cpu.usagePercent, memory: system.memory, errors: system.errors }, workspaces: { enabled: workspaces.filter((item) => item.enabled).length, total: workspaces.length }, routines: { enabled: routines.filter((item) => item.enabled).length, total: routines.length }, starkKnowledgeSystem: "conhecimento, roadmaps, pesquisa, evolução e lacunas", learningEngine: "ativo com eventos auditáveis", aiCore: "online quando Ollama disponível", automationCore: "rotinas autorizadas", writeAccess: "somente ações previamente autorizadas" }; } },
    { name: "get_azriel_version", domain: "Azriel", description: "Versão atual", readonly: true, execute: async () => ({ version: "0.8.3", name: "Learning Engine" }) },
    ...
    { name: "get_component_semantics", domain: "Assembly Intelligence", description: "Semântica e função do componente identificado ou selecionado", readonly: true, permission: "read", execute: (input) => engineering.getComponentSemantics(engineeringReference(input)) },
    { name: "get_subsystems", domain: "Assembly Intelligence", description: "Lista subsistemas persistidos do modelo atual", readonly: true, permission: "read", execute: () => engineering.getSubsystems() },
    { name: "get_subsystem_components", domain: "Assembly Intelligence", description: "Componentes pertencentes a um subsistema", readonly: true, permission: "read", execute: (input) => engineering.getSubsystemComponents(engineeringReference(input)) },
    { name: "get_component_relationships", domain: "Assembly Intelligence", description: "Relações persistidas do componente identificado ou selecionado", readonly: true, permission: "read", execute: (input) => engineering.getComponentRelationships(engineeringReference(input)) },
    { name: "get_unclassified_components", domain: "Assembly Intelligence", description: "Componentes ainda não classificados integralmente", readonly: true, permission: "read", execute: () => engineering.getUnclassifiedComponents() },
    { name: "get_semantic_coverage", domain: "Assembly Intelligence", description: "Cobertura semântica dos componentes selecionáveis", readonly: true, permission: "read", execute: () => engineering.getSemanticCoverage() },
    { name: "get_assembly_graph_summary", domain: "Assembly Intelligence", description: "Resumo do grafo semântico da montagem", readonly: true, permission: "read", execute: () => engineering.getAssemblyGraphSummary() },
