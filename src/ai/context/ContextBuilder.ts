import type { AIToolPermission, AIToolResult, RoutedIntent } from "../../types";
...
export class ContextBuilder {
  ...
  async build(query: string, intent: RoutedIntent, onTool?: (domain: string, permission: AIToolPermission) => void): Promise<BuiltContext> {
    const results: AIToolResult[] = [];
    for (const name of intent.tools) {
      const tool = this.registry.get(name);
      onTool?.(tool.domain, tool.permission ?? "read");
      results.push(await this.registry.execute(name, { query, term: intent.term, factor: intent.factor, delta: intent.delta }));
    }
